...
std::vector<int> mergeTwoDescendingPiles(std::vector<int>& pile1, std::vector<int>& pile2) {
    std::vector<int> merged;
    // NOTE: We use int instead of size_t to avoid additional if-statements and breaks
...
int findPile(std::vector<std::vector<int>>& piles, int value) {
...
std::vector<std::vector<int>> createPiles(std::vector<int>& arr) {
    std::vector<std::vector<int>> piles;
    for (int value : arr) {
        int pileIndex = findPile(piles, value);
...
std::vector<int> patienceSort(std::vector<int>& arr) {
    ...
    // Step 1: Create piles
    std::vector<std::vector<int>> piles;
    piles = createPiles(arr);

    // Step 2: Merge piles into sorted array
    std::vector<int> sortedArr;
    sortedArr = mergePiles(piles);
