...
template <bool UseHint, bool EnableValleyRescue = false, bool EnableOppositeGameOverflowRescue = false, typename T, typename Less = std::less<T>>
FrozenInsertionResult<T> simulatePatienceInsertionBlueprintEarlyFreezeImpl(
    ...
    auto processFrozen = [&](const T& previous, const T& value, std::size_t i) {
        ...
        if (overflow) {
            if constexpr (EnableOppositeGameOverflowRescue) {
                // Preserve the normal directional choice. Only when that frozen
                // game would require a new pile, try an already-existing pile
                // in the opposite frozen game before deferring to overflow.
                if (descendingMode) {
                    const bool canUseAsc = !result.ascTails.empty() &&
                        !less(value, result.ascTails.back());
                    if (canUseAsc) {
                        simulateInsertValueAscendingPiles<UseHint>(
                            result.ascTails, lastPileIndexAscending, value, i,
                    ...
                    const bool canUseDesc = !result.descTails.empty() &&
                        !less(result.descTails.back(), value);
                    if (canUseDesc) {
                        simulateInsertValueDescendingPiles<UseHint>(
                            result.descTails, lastPileIndexDescending, value, i,
...
template <typename T, typename Less = std::less<T>>
FrozenInsertionResult<T> simulatePatienceInsertionBlueprintEarlyFreezeOppositeOverflowRescue(
    const std::vector<T>& arr,
    Less less = Less{},
    FreezePolicy freezePolicy = FreezePolicy::fixed_half,
    bool naturalRunRoute = false,
    ...
    const bool valleyRoute = enableValleyRescue && arr.size() >= 10000 && prefixPiles >= 3 && prefixPiles <= 12;
    if (noHintRoute) {
        if (valleyRoute) return simulatePatienceInsertionBlueprintEarlyFreezeImpl<false, true, true>(
            arr, less, freezePolicy, naturalRunRoute, naturalRequireFirstContinuationLong, enableCoherentValuePileCache, freezePercent, sampleOverflowShape);
        return simulatePatienceInsertionBlueprintEarlyFreezeImpl<false, false, true>(
            arr, less, freezePolicy, naturalRunRoute, naturalRequireFirstContinuationLong, enableCoherentValuePileCache, freezePercent, sampleOverflowShape);
    }
    if (valleyRoute) return simulatePatienceInsertionBlueprintEarlyFreezeImpl<true, true, true>(
        arr, less, freezePolicy, naturalRunRoute, naturalRequireFirstContinuationLong, enableCoherentValuePileCache, freezePercent, sampleOverflowShape);
    return simulatePatienceInsertionBlueprintEarlyFreezeImpl<true, false, true>(
        arr, less, freezePolicy, naturalRunRoute, naturalRequireFirstContinuationLong, enableCoherentValuePileCache, freezePercent, sampleOverflowShape);
}


template <typename T, typename Less = std::less<T>>
FrozenInsertionResult<T> simulatePatienceInsertionBlueprintEarlyFreezeRandomPrefixGatedOppositeOverflowRescue(
    const std::vector<T>& arr,
    Less less = Less{},
    FreezePolicy freezePolicy = FreezePolicy::fixed_half,
    bool naturalRunRoute = false,
    ...
    // E587 refinement: the opposite-game rescue is only admitted on the
    // existing random-like prefix route. Structured/non-random routes compile
    // through the ordinary frozen insertion path with no rescue behavior.
    if (noHintRoute) {
        if (valleyRoute) return simulatePatienceInsertionBlueprintEarlyFreezeImpl<false, true, true>(
            arr, less, freezePolicy, naturalRunRoute, naturalRequireFirstContinuationLong, enableCoherentValuePileCache, freezePercent, sampleOverflowShape);
        return simulatePatienceInsertionBlueprintEarlyFreezeImpl<false, false, true>(
    ...
    if (valleyRoute) return simulatePatienceInsertionBlueprintEarlyFreezeImpl<true, true, false>(
        arr, less, freezePolicy, naturalRunRoute, naturalRequireFirstContinuationLong, enableCoherentValuePileCache, freezePercent, sampleOverflowShape);
    return simulatePatienceInsertionBlueprintEarlyFreezeImpl<true, false, false>(
        arr, less, freezePolicy, naturalRunRoute, naturalRequireFirstContinuationLong, enableCoherentValuePileCache, freezePercent, sampleOverflowShape);
}
