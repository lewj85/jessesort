#ifndef JESSESORT_E691_PHASE_RANGE_BACKBONE_ENGINE_H
#define JESSESORT_E691_PHASE_RANGE_BACKBONE_ENGINE_H
...
namespace jessesort::simulated_phase_range_engine_e691 {
...
// E688: phase-local context gate. Pressure latches independently per game,
// but a phase-range bailout is armed only once BOTH games have independently
// reached T=9. A pressured descending game therefore cannot condemn a healthy
// ascending game or the rest of the phase by itself.
struct T9PressureState {
    std::uint32_t ascBits=0,descBits=0;
    std::size_t ascSeen=0,descSeen=0;
    bool ascPressured=false,descPressured=false;
    bool overflowActive=false,overflowDesc=false;
...
inline bool t9PressureDivertToOverflow(bool desc,std::vector<uint32_t>& blueprint,std::size_t i) noexcept {
    auto* st=g_t9PressureState;
    if(!st||t9PressureTripped()) return false;
    const bool thisPressured=desc?st->descPressured:st->ascPressured;
    const bool otherPressured=desc?st->ascPressured:st->descPressured;
    if(!thisPressured||otherPressured) return false;
    if(!st->overflowActive){ st->overflowActive=true; st->overflowDesc=desc; }
    if(st->overflowDesc!=desc) return false;
    blueprint[i]=OVERFLOW_TAG; ++st->overflowCount;
    return true;
}
inline void t9PressureRecord(bool desc,bool created,std::size_t pileCount,std::size_t i) noexcept {
    ...
    auto& pressured=desc?st->descPressured:st->ascPressured;
    bits=(bits<<1)|(created?1u:0u); ++seen;
    if(!pressured && pileCount>=32 && seen>=32 && std::popcount(bits)>=9) pressured=true;
    if(st->ascPressured && st->descPressured) st->tripIndex=i+1;
}
...
inline void simulateInsertValueDescendingPiles(
    ...
    if (t9PressureDivertToOverflow(true, blueprint, originalIndex)) return;
...
inline void simulateInsertValueAscendingPiles(
    ...
    if (t9PressureTripped()) return;
    if (t9PressureDivertToOverflow(false, blueprint, originalIndex)) return;
...
inline void simulateInsertAdjacentEquivalent(
    ...
    if (t9PressureTripped()) return;
    if (t9PressureDivertToOverflow(descendingMode, result.blueprint, originalIndex)) return;
    if (descendingMode) {
...
JESSESORT_CONTINUE_NOINLINE void continuePatienceInsertionCoherentCache(
    ...
    auto process = [&](const T& previous, const T& value, std::size_t i) {
        ...
        if (t9PressureDivertToOverflow(descendingMode, result.blueprint, i)) return;
...
inline void continuePatienceInsertionNaturalRuns(
    ...
    while (i < n && !t9PressureTripped()) {
        ...
        const bool pressureBlocksBatch = g_t9PressureState &&
            (asc ? g_t9PressureState->ascPressured : g_t9PressureState->descPressured);
        if (len >= minRunLength && !pressureBlocksBatch) {
            if (asc) {
...
SimulatedInsertionResult<T> simulatePatienceInsertionBlueprintImpl(
    ...
    const bool confirmedShortSameDirectionPrefix =
        ...
        jessesort::simulated_phase_range_engine_e691::confirmedShortSameDirectionPrefix(
...
// E690: reconstruct a phase-local blueprint with one deferred T=9 overflow run.
// Normal desc/asc pile geometry is preserved; OVERFLOW_TAG values are gathered
// contiguously, sorted once, and exposed as one additional run.
template <typename T, typename Less>
std::vector<std::size_t> reconstructTaggedBlueprintWithSinglePressureOverflow(
    std::span<const T> arr,
    const std::vector<uint32_t>& blueprint,
    std::vector<std::size_t> ascCounts,
    std::vector<std::size_t> descCounts,
    ...
    const std::size_t nd=descCounts.size(), na=ascCounts.size();
    const std::size_t nr=nd+na;
    std::vector<std::size_t> start(nr+1,0);
    std::size_t out=0;
    for(std::size_t p=0;p<nd;++p){ start[p]=out; out+=descCounts[p]; }
    for(std::size_t p=0;p<na;++p){ const std::size_t r=nd+p; start[r]=out; out+=ascCounts[p]; }
    start[nr]=out; assert(out==normalCount);
    if constexpr(std::is_default_constructible_v<T>) tmp.resize(n); else tmp.assign(arr.begin(),arr.end());
    std::vector<std::size_t> cursor(nr);
    for(std::size_t p=0;p<nd;++p) cursor[p]=start[p+1]-1; // descending piles normalize by reversal
    for(std::size_t p=0;p<na;++p) cursor[nd+p]=start[nd+p];
    std::size_t ov=0;
    for(std::size_t i=0;i<n;++i){
        const uint32_t tag=blueprint[i];
        if(tag==OVERFLOW_TAG){ tmp[normalCount+ov++]=arr[i]; continue; }
        if(isDescTag(tag)) tmp[cursor[static_cast<std::size_t>(localPileId(tag))]--]=arr[i];
        else tmp[cursor[nd+static_cast<std::size_t>(localPileId(tag))]++]=arr[i];
    }
    assert(ov==overflowCount);
    if(overflowCount){
        // Retained frozen-live policy: preserve only strong (>=32) natural
        // overflow runs. Weak/fragmented overflow is normalized in 32-element
        // bands so tiny direction changes do not explode the merge leaf count.
        constexpr std::size_t MinNatural=32;
        std::size_t first=normalCount;
        while(first<n){
            std::size_t naturalEnd=first+1;
            bool descending=false;
            if(naturalEnd<n){
                if(less(tmp[naturalEnd],tmp[first])) descending=true;
                ++naturalEnd;
                if(descending){ while(naturalEnd<n && !less(tmp[naturalEnd-1],tmp[naturalEnd])) ++naturalEnd; }
                else { while(naturalEnd<n && !less(tmp[naturalEnd],tmp[naturalEnd-1])) ++naturalEnd; }
            }
            const std::size_t naturalLen=naturalEnd-first;
            if(naturalLen>=MinNatural){
                if(descending) std::reverse(tmp.begin()+static_cast<std::ptrdiff_t>(first),
                                            tmp.begin()+static_cast<std::ptrdiff_t>(naturalEnd));
                start.push_back(naturalEnd);
                first=naturalEnd;
            } else {
                const std::size_t bandEnd=std::min(n,first+MinNatural);
                for(std::size_t k=first+1;k<bandEnd;++k){
                    T value=std::move(tmp[k]); std::size_t pos=k;
                    while(pos>first && less(value,tmp[pos-1])){ tmp[pos]=std::move(tmp[pos-1]); --pos; }
                    tmp[pos]=std::move(value);
                }
                start.push_back(bandEnd);
                first=bandEnd;
            }
        }
    }
...
void sortImplCore(std::span<T> arr, Less less, bool enableNaturalRunRoute,
    ...
    if (t9Pressure.tripIndex != std::numeric_limits<std::size_t>::max() && t9Pressure.tripIndex < arr.size()) {
        ...
        auto runStart = t9Pressure.overflowCount
            ? reconstructTaggedBlueprintWithSinglePressureOverflow(
                std::span<const T>(prefixSource.data(),prefixSource.size()), sim.blueprint,
                std::move(sim.ascCounts),std::move(sim.descCounts),t9Pressure.overflowCount,prefixTmp,less)
            : reconstructTaggedBlueprintNormalizedForSimulated(
                std::span<const T>(prefixSource.data(),prefixSource.size()),std::move(sim.blueprint),
                std::move(sim.ascCounts),std::move(sim.descCounts),prefixTmp,false,true,true,true);
        if constexpr (specializedIntegralEligible<T, Less> && std::is_default_constructible_v<T>) {
            highEntropyQuickSort(arr.data()+static_cast<std::ptrdiff_t>(prefixLen),arr.size()-prefixLen,
                2*static_cast<int>(std::bit_width(arr.size()-prefixLen)),less,false,T{},false,true,nullptr,false);
        } else std::sort(arr.begin()+static_cast<std::ptrdiff_t>(prefixLen),arr.end(),less);
        std::vector<T> combined; if constexpr(std::is_default_constructible_v<T>) combined.resize(arr.size()); else combined.assign(arr.begin(),arr.end());
        std::move(prefixTmp.begin(),prefixTmp.end(),combined.begin());
        std::move(arr.begin()+static_cast<std::ptrdiff_t>(prefixLen),arr.end(),combined.begin()+static_cast<std::ptrdiff_t>(prefixLen));
        ...
        mergeRunsAdjacentPairsEndsToSpan(combined,arr,ends,less,false,true);
        return;
    }
    ...
    std::vector<std::size_t> runStart;
    if (t9Pressure.overflowCount) {
        runStart = reconstructTaggedBlueprintWithSinglePressureOverflow(
            std::span<const T>(arr.data(),arr.size()), sim.blueprint,
            std::move(sim.ascCounts),std::move(sim.descCounts),t9Pressure.overflowCount,tmp,less);
    } else {
        runStart = reconstructTaggedBlueprintNormalizedForSimulated(
