#ifndef JESSESORT_SIMULATED_DIRECT_DENSE_GHOST_H
#define JESSESORT_SIMULATED_DIRECT_DENSE_GHOST_H
...
namespace jessesort::simulated_direct_dense_ghost {
...
SimulatedInsertionResult<T> simulatePatienceInsertionBlueprintImpl(
    ...
    const bool confirmedShortSameDirectionPrefix =
        ...
        jessesort::simulated_direct_dense_ghost::confirmedShortSameDirectionPrefix(
...
// E709 diagnostic only: emit reconstructed-run cross-game geometry without
// changing reconstruction or merge behavior. Enabled by JESSESORT_E709_CENSUS=1.
template <typename T, typename Less>
inline void e709EmitCrossGameCensus(
    const std::vector<T>& tmp, const std::vector<std::size_t>& start,
    std::size_t normalDesc, std::size_t totalDesc,
    std::size_t normalAsc, std::size_t totalAsc, Less less) {
    const char* on = std::getenv("JESSESORT_E709_CENSUS");
    if (!on || *on != '1' || start.size() < 2) return;
    const char* fam = std::getenv("JESSESORT_E709_FAMILY");
    const char* ns = std::getenv("JESSESORT_E709_N");
    const std::string family = fam ? fam : "unknown";
    const std::string nlabel = ns ? ns : "0";
    struct R { std::size_t id,len; bool desc,ghost; const T* lo; const T* hi; };
    std::vector<R> runs; runs.reserve(start.size()-1);
    for (std::size_t r=0;r+1<start.size();++r) {
        if (start[r]==start[r+1]) continue;
        bool d=r<totalDesc; std::size_t local=d?r:r-totalDesc;
        bool g=d ? local>=normalDesc : local>=normalAsc;
        runs.push_back({r,start[r+1]-start[r],d,g,&tmp[start[r]],&tmp[start[r+1]-1]});
    }
    auto disjoint=[&](const R&a,const R&b){ return !less(*b.hi,*a.lo) ? (!less(*a.lo,*b.hi) && false) : false; };
    auto stitch=[&](const R&a,const R&b){
        return !less(*b.lo,*a.hi) || !less(*a.lo,*b.hi);
    };
    // Above uses <= semantics under strict weak ordering: a.hi <= b.lo OR b.hi <= a.lo.
    auto canStitch=[&](const R&a,const R&b){
        return !less(*b.lo,*a.hi) || !less(*a.lo,*b.hi);
    };
    std::vector<R> D,A; for(auto &r:runs)(r.desc?D:A).push_back(r);
    unsigned long long cross=0,crossSt=0,crossWeight=0,crossStWeight=0;
    unsigned long long crossNormal=0,crossNormalSt=0,crossNormalWeight=0,crossNormalStWeight=0;
    std::vector<char> dh(D.size()),ah(A.size());
    for(std::size_t i=0;i<D.size();++i) for(std::size_t j=0;j<A.size();++j){
        ++cross; crossWeight += D[i].len + A[j].len;
        bool st=canStitch(D[i],A[j]);
        if(st){++crossSt;crossStWeight += D[i].len+A[j].len;dh[i]=ah[j]=1;}
        if(!D[i].ghost&&!A[j].ghost){++crossNormal;crossNormalWeight+=D[i].len+A[j].len;if(st){++crossNormalSt;crossNormalStWeight+=D[i].len+A[j].len;}}
    }
    auto simulate=[&](const char* label,std::vector<R> cur){
        unsigned long long stitches=0, work=0, elems=0; std::size_t tier=0;
        std::string tiers;
        while(cur.size()>1){
            std::vector<R> next; next.reserve((cur.size()+1)/2);
            unsigned long long ts=0,tw=0;
            for(std::size_t i=0;i<cur.size();i+=2){
                if(i+1==cur.size()){next.push_back(cur[i]);continue;}
                R x=cur[i],y=cur[i+1]; bool st=canStitch(x,y);
                R z=x; z.len=x.len+y.len; z.ghost=x.ghost||y.ghost;
                z.lo= less(*y.lo,*x.lo)?y.lo:x.lo; z.hi=less(*x.hi,*y.hi)?y.hi:x.hi;
                if(st){++stitches;++ts;work+=z.len;tw+=z.len;elems+=z.len;}
                next.push_back(z);
            }
            if(!tiers.empty())tiers+=';'; tiers+=std::to_string(tier)+":"+std::to_string(ts)+":"+std::to_string(tw);
            cur.swap(next);++tier;
        }
        std::cerr<<"E709_LAYOUT,"<<family<<','<<nlabel<<','<<label<<','<<runs.size()<<','<<D.size()<<','<<A.size()<<','<<stitches<<','<<work<<','<<tiers<<"\n";
    };
    std::vector<R> current=runs; simulate("current",current);
    auto pairlayout=[&](bool opposite,bool normalized){
        std::vector<R> out; std::vector<char> usedD(D.size()),usedA(A.size());
        std::size_t m=std::min(D.size(),A.size());
        for(std::size_t k=0;k<m;++k){
            std::size_t di=k, ai=k;
            if(opposite) di=D.size()-1-k;
            if(normalized){
                if(D.size()<=A.size()) { di=k; ai=(m<=1?0: (k*(A.size()-1)+(m-1)/2)/(m-1)); while(ai<A.size()&&usedA[ai])++ai; if(ai>=A.size()){ai=A.size();while(ai&&usedA[ai-1])--ai;--ai;} }
                else { ai=k; di=(m<=1?0: (k*(D.size()-1)+(m-1)/2)/(m-1)); while(di<D.size()&&usedD[di])++di; if(di>=D.size()){di=D.size();while(di&&usedD[di-1])--di;--di;} }
            }
            if(di<D.size()&&ai<A.size()&&!usedD[di]&&!usedA[ai]){out.push_back(D[di]);out.push_back(A[ai]);usedD[di]=usedA[ai]=1;}
        }
        for(std::size_t i=0;i<D.size();++i)if(!usedD[i])out.push_back(D[i]);
        for(std::size_t i=0;i<A.size();++i)if(!usedA[i])out.push_back(A[i]);
        return out;
    };
    simulate("same_rank",pairlayout(false,false));
    simulate("opposite_rank",pairlayout(true,false));
    simulate("normalized_rank",pairlayout(false,true));
    unsigned long long partnerD=std::accumulate(dh.begin(),dh.end(),0ULL), partnerA=std::accumulate(ah.begin(),ah.end(),0ULL);
    std::cerr<<"E709_CROSS,"<<family<<','<<nlabel<<','<<runs.size()<<','<<normalDesc<<','<<totalDesc<<','<<normalAsc<<','<<totalAsc<<','
             <<cross<<','<<crossSt<<','<<crossWeight<<','<<crossStWeight<<','<<partnerD<<','<<partnerA<<','
             <<crossNormal<<','<<crossNormalSt<<','<<crossNormalWeight<<','<<crossNormalStWeight<<"\n";
}
...
void sortImplCoreWithCheckpoint(std::vector<T>& arr, Less less, bool enableNaturalRunRoute,
    ...
    const std::size_t e709NormalAsc = sim.ascCounts.size();
    const std::size_t e709NormalDesc = sim.descCounts.size();
    const std::size_t e709TotalAsc = e709NormalAsc + sim.detachedAscCounts.size();
    const std::size_t e709TotalDesc = e709NormalDesc + sim.detachedDescCounts.size();
    std::vector<std::size_t> runStart =
    ...
    e709EmitCrossGameCensus(tmp, runStart, e709NormalDesc, e709TotalDesc, e709NormalAsc, e709TotalAsc, less);
