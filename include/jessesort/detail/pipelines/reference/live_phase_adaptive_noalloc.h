...
// Maintained performance-oriented allocation-free live-phase pipeline.
// E728 rebased adaptive noalloc onto the E691->E701->E706->E723->E726
// live-phase front end. E729 retained one size-gated, whole-input mature
// noalloc long-natural-run attempt before the live-phase loop.
// Strict bounded noalloc is intentionally separate and does not use this path.
...
namespace jessesort::allocation_free_live_phase_adaptive {
...
template<class T, class Less, bool Adaptive, bool SkipGlobalPrepass = false>
void sortImpl(std::vector<T>& values, Less less) {
    ...
    if constexpr (Adaptive && !SkipGlobalPrepass) {
