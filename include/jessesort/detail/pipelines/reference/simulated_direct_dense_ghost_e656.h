#ifndef JESSESORT_SIMULATED_DIRECT_DENSE_GHOST_E656_LOCAL_FUSION_H
#define JESSESORT_SIMULATED_DIRECT_DENSE_GHOST_E656_LOCAL_FUSION_H
...
namespace jessesort::simulated_direct_dense_ghost_e656_local_fusion {
inline bool g_e725_disable_sparse_disorder = false;
...
// Retained dense-ghost tag space used only for proven monotone runs spanning >=3 live pile regions.
inline constexpr std::size_t DENSE_GHOST_MIN_RUN_LENGTH = 32; // Merge-economics floor for detached runs.
// live pile regions under the E558 coherence-gated continuation.
static constexpr uint32_t DETACHED_BIT = 0x40000000u;
static constexpr uint32_t DETACHED_ID_MASK = 0x3fffffffu;
inline uint32_t makeDetachedAscTag(uint32_t run) { return DETACHED_BIT | run; }
inline uint32_t makeDetachedDescTag(uint32_t run) { return DESC_BIT | DETACHED_BIT | run; }
inline bool isDetachedTag(uint32_t tag) { return (tag & DETACHED_BIT) != 0; }
inline uint32_t detachedRunId(uint32_t tag) { return tag & DETACHED_ID_MASK; }
...
struct SimulatedInsertionResult {
    ...
    std::vector<std::size_t> detachedAscCounts;
    std::vector<std::size_t> detachedDescCounts;
...
// E686: per-game T=9 rolling pile-pressure safety valve.
struct T9PressureState {
    std::uint32_t ascBits=0, descBits=0;
    std::size_t ascSeen=0, descSeen=0;
    std::size_t tripIndex=std::numeric_limits<std::size_t>::max();
};
inline thread_local T9PressureState* g_t9PressureState=nullptr;
inline bool t9PressureTripped() noexcept { return g_t9PressureState && g_t9PressureState->tripIndex!=std::numeric_limits<std::size_t>::max(); }
inline void t9PressureRecord(bool desc,bool created,std::size_t pileCount,std::size_t i) noexcept {
    auto* st=g_t9PressureState; if(!st||t9PressureTripped()) return;
    auto& bits=desc?st->descBits:st->ascBits; auto& seen=desc?st->descSeen:st->ascSeen;
    bits=(bits<<1)|(created?1u:0u); ++seen;
    if(pileCount>=32 && seen>=32 && std::popcount(bits)>=9) st->tripIndex=i+1;
}
...
inline void simulateInsertValueDescendingPiles(
    ...
    t9PressureRecord(true,t9Created,descCounts.size(),originalIndex);
}
...
inline void simulateInsertValueAscendingPiles(
    ...
    const std::size_t pileIndex = findAscendingPileWithTails(
        tails, lastPileIndex, value, less);
    const bool t9Created = pileIndex >= ascCounts.size();
    if (pileIndex < tails.size()) {
        tails[pileIndex] = value;
        ++ascCounts[pileIndex];
    } else {
    ...
    t9PressureRecord(false,t9Created,ascCounts.size(),originalIndex);
}
...
inline void simulateInsertAdjacentEquivalent(
    ...
    if (descendingMode) {
        ...
        t9PressureRecord(true,false,result.descCounts.size(),originalIndex);
    } else {
        ...
        t9PressureRecord(false,false,result.ascCounts.size(),originalIndex);
    }
}
...
JESSESORT_CONTINUE_NOINLINE void continuePatienceInsertionCoherentCache(
    ...
    auto process = [&](const T& previous, const T& value, std::size_t i) {
        ...
        if (e.valid && e.key == key && e.pile < tails.size()) {
            ...
            t9PressureRecord(descendingMode,false,counts.size(),i);
            return;
        }
        ...
        const bool t9Created = p >= counts.size();
        if (p < tails.size()) {
            const T old = tails[p];
        ...
        t9PressureRecord(descendingMode,t9Created,counts.size(),i);
    };
...
JESSESORT_CONTINUE_NOINLINE void continuePatienceInsertionNoHint(
    ...
    auto process = [&](const T& previous, const T& value, std::size_t i) {
        ...
        if (descendingMode) {
            ...
            const bool t9Created = pileIndex >= result.descCounts.size();
            ...
            t9PressureRecord(true,t9Created,result.descCounts.size(),i);
        } else {
            const std::size_t pileIndex =
                findAscendingPileWithTailsNoHint(result.ascTails, value, less);
            const bool t9Created = pileIndex >= result.ascCounts.size();
            ...
            t9PressureRecord(false,t9Created,result.ascCounts.size(),i);
        }
    };
...
// E586 correctness helper: a detached run has no live-pile owner.  Keep any
// immediately trailing comparator-equivalent plateau with the ghost so E117
// cannot reuse a stale lastPileIndex on the next source element.  Keep this
// cold relative to the continuation loop to minimize hot-path code-shape cost.
#if defined(__GNUC__) || defined(__clang__)
#define JESSESORT_GHOST_NOINLINE __attribute__((noinline))
#else
#define JESSESORT_GHOST_NOINLINE
#endif
template <typename T, typename Less>
JESSESORT_GHOST_NOINLINE std::size_t extendDetachedRunThroughEquivalentPlateau(
    const std::vector<T>& arr, std::size_t end, Less less) {
    while (end < arr.size() && !less(arr[end - 1], arr[end]) &&
           !less(arr[end], arr[end - 1])) ++end;
    return end;
}
#undef JESSESORT_GHOST_NOINLINE
        ...
        if (len >= minRunLength) {
            if (asc) {
                ...
                if (firstPile == lastPile) {
                    ...
                    const std::size_t pieces = firstPile > lastPile ? firstPile - lastPile + 1 : lastPile - firstPile + 1;
                    if (pieces >= 3 && len >= DENSE_GHOST_MIN_RUN_LENGTH) {
                        const std::size_t ghostEnd =
                            extendDetachedRunThroughEquivalentPlateau(arr, end, less);
                        const std::size_t ghost = result.detachedAscCounts.size();
                        assert(ghost <= DETACHED_ID_MASK);
                        result.detachedAscCounts.push_back(ghostEnd - i);
                        const uint32_t tag = makeDetachedAscTag(static_cast<uint32_t>(ghost));
                        std::fill(result.blueprint.begin() + static_cast<std::ptrdiff_t>(i),
                                  result.blueprint.begin() + static_cast<std::ptrdiff_t>(ghostEnd), tag);
                        end = ghostEnd;
                        descendingMode = false;
                        overridePending = false;
                        batched = true;
                ...
                if (firstPile == lastPile) {
                    ...
                    if (pieces >= 3 && len >= DENSE_GHOST_MIN_RUN_LENGTH) {
                        const std::size_t ghostEnd =
                            extendDetachedRunThroughEquivalentPlateau(arr, end, less);
                        const std::size_t ghost = result.detachedDescCounts.size();
                        assert(ghost <= DETACHED_ID_MASK);
                        result.detachedDescCounts.push_back(ghostEnd - i);
                        const uint32_t tag = makeDetachedDescTag(static_cast<uint32_t>(ghost));
                        std::fill(result.blueprint.begin() + static_cast<std::ptrdiff_t>(i),
                                  result.blueprint.begin() + static_cast<std::ptrdiff_t>(ghostEnd), tag);
                        end = ghostEnd;
                        descendingMode = true;
...
SimulatedInsertionResult<T> simulatePatienceInsertionBlueprintImpl(
    ...
    const bool confirmedShortSameDirectionPrefix =
        ...
        jessesort::simulated_direct_dense_ghost_e656_local_fusion::confirmedShortSameDirectionPrefix(
    ...
    if constexpr (std::is_trivially_copyable_v<T> && sizeof(T) <= 2 * sizeof(void*)) {
        ...
        if (i < n) {
            if (useValleyRescue) continuePatienceInsertionValleyRescue<EnableAdjacentEqualFastPath>(
                ...
                arr, i, result, lastPileIndexAscending, lastPileIndexDescending, descendingMode, less);
            else if (useSplit) continuePatienceInsertionNaturalRuns<EnableAdjacentEqualFastPath>(
                arr, i, result, lastPileIndexAscending, lastPileIndexDescending, descendingMode, less, 8);
...
std::vector<std::size_t> reconstructTaggedBlueprintToTemp_WithSplitCounts(
    ...
    const std::size_t n = arr.size();
    const std::size_t normalAscPiles = ascCounts.size();
    const std::size_t normalDescPiles = descCounts.size();
    ascCounts.insert(ascCounts.end(), detachedAscCounts.begin(), detachedAscCounts.end());
    descCounts.insert(descCounts.end(), detachedDescCounts.begin(), detachedDescCounts.end());
    ...
    auto decodeRun = [&](uint32_t tag) -> std::size_t {
        const bool desc = isDescTag(tag);
        std::size_t local;
        if (isDetachedTag(tag)) {
            const std::size_t ghost = static_cast<std::size_t>(detachedRunId(tag));
            local = (desc ? normalDescPiles : normalAscPiles) + ghost;
        } else {
            local = static_cast<std::size_t>(localPileId(tag));
        }
        return desc ? local : numDescPiles + local;
    };
...
std::vector<std::size_t> reconstructTaggedBlueprintNormalizedForSimulated(
    ...
    if (!decodeBulkTagsOnTheFly) {
        for (std::size_t i = 0; i < n; ++i) {
            blueprint[i] = static_cast<uint32_t>(decodeRun(blueprint[i]));
        }
    }
...
// E601 reconstructed checkpoint: compact exact alphabetic scheduler minimizing
// internal merge-output writes. Definition intentionally lives at the end of
// the header so inactive hot paths are minimally perturbed.
template <typename T, typename Less>
#if defined(__GNUC__) || defined(__clang__)
__attribute__((noinline, cold))
#endif
void mergeRunsE601InternalWriteTree(
    std::vector<T>& tmp, std::vector<T>& arr,
    std::vector<std::size_t> runStart, Less less);
...
// Common physical/simulated/simulated-inplace adj-pair merge driver selected by E115.
template <class T, class Less>
...
// E656 candidate: first-level local three-run virtual fusion.  The route is
// decided independently for each reconstructed triple from comparator-visible
// local geometry.  Subsequent levels return to the mature E601 pairwise merge.
template <class T, class Less>
inline bool e656Equivalent(const T& a, const T& b, Less less) {
    return !less(a,b) && !less(b,a);
}

template <class T, class Less>
inline unsigned e656EqualityScore(const std::vector<T>& v,
                                  std::size_t a, std::size_t b,
                                  std::size_t c, std::size_t d, Less less) {
    unsigned eq=0;
    constexpr unsigned K=5;
    auto one=[&](std::size_t lo,std::size_t hi){
        const std::size_t n=hi-lo;
        if(n<2) return;
        for(unsigned j=0;j<K-1;++j){
            const std::size_t x=lo+(n-1)*j/(K-1);
            const std::size_t y=lo+(n-1)*(j+1)/(K-1);
            eq += e656Equivalent(v[x],v[y],less);
        }
    };
    one(a,b); one(b,c); one(c,d);
    return eq;
}

template <class T, class Less>
inline std::size_t e656OpeningBulkSpan(const std::vector<T>& v,
                                       std::size_t a,std::size_t b,
                                       std::size_t c,std::size_t d, Less less) {
    bool fromB;
    if(a==b) fromB=true;
    else if(b==c) fromB=false;
    else fromB=less(v[b],v[a]);
    const std::size_t ab=fromB?b:a;
    if(c<d && less(v[c],v[ab])) {
        const T& q=v[ab];
        auto e=std::lower_bound(v.begin()+static_cast<std::ptrdiff_t>(c),
                                v.begin()+static_cast<std::ptrdiff_t>(d),q,less);
        return static_cast<std::size_t>(e-(v.begin()+static_cast<std::ptrdiff_t>(c)));
    }
    if(fromB) {
        std::size_t e=c;
        if(a<b) {
            auto it=std::lower_bound(v.begin()+static_cast<std::ptrdiff_t>(b),
                                     v.begin()+static_cast<std::ptrdiff_t>(e),v[a],less);
            e=std::min(e,static_cast<std::size_t>(it-v.begin()));
        }
        if(c<d) {
            auto it=std::upper_bound(v.begin()+static_cast<std::ptrdiff_t>(b),
                                     v.begin()+static_cast<std::ptrdiff_t>(e),v[c],less);
            e=std::min(e,static_cast<std::size_t>(it-v.begin()));
        }
        return e-b;
    }
    std::size_t e=b;
    if(b<c) {
        auto it=std::upper_bound(v.begin()+static_cast<std::ptrdiff_t>(a),
                                 v.begin()+static_cast<std::ptrdiff_t>(e),v[b],less);
        e=std::min(e,static_cast<std::size_t>(it-v.begin()));
    }
    if(c<d) {
        auto it=std::upper_bound(v.begin()+static_cast<std::ptrdiff_t>(a),
...
template <class T, class Less>
inline bool e656PreferGallop(const std::vector<T>& v,
                             std::size_t a,std::size_t b,
                             std::size_t c,std::size_t d, Less less) {
    if(e656EqualityScore(v,a,b,c,d,less)>=6) return true;
    return d-a>=512 && e656OpeningBulkSpan(v,a,b,c,d,less)>=48;
}

template <class T, class Less>
inline void e656ScalarTriple(std::vector<T>& in,std::vector<T>& out,
                             std::size_t a,std::size_t b,std::size_t c,std::size_t d,Less less) {
    std::size_t x=a,y=b,z=c,o=a; bool have=false,fromB=false;
    while((x<b||y<c)&&z<d){
        if(!have){ if(x==b) fromB=true; else if(y==c) fromB=false; else fromB=less(in[y],in[x]); have=true; }
        const std::size_t ab=fromB?y:x;
        if(less(in[z],in[ab])) out[o++]=std::move(in[z++]);
        else { out[o++]=std::move(in[ab]); if(fromB)++y;else++x; have=false; }
    }
    while(x<b||y<c){
        if(x==b) out[o++]=std::move(in[y++]);
        else if(y==c) out[o++]=std::move(in[x++]);
        else if(less(in[y],in[x])) out[o++]=std::move(in[y++]);
        else out[o++]=std::move(in[x++]);
    }
    while(z<d) out[o++]=std::move(in[z++]);
}

template <class T, class Less>
inline void e656GallopTriple(std::vector<T>& in,std::vector<T>& out,
                             std::size_t a,std::size_t b,std::size_t c,std::size_t d,Less less) {
    constexpr unsigned TRIGGER=16; constexpr std::size_t MINSPAN=16;
    std::size_t x=a,y=b,z=c,o=a; bool have=false,fromB=false; int last=-1; unsigned streak=0;
    while((x<b||y<c)&&z<d){
        if(!have){if(x==b)fromB=true;else if(y==c)fromB=false;else fromB=less(in[y],in[x]);have=true;}
        const std::size_t ab=fromB?y:x; int src;
        if(less(in[z],in[ab])){src=2;out[o++]=std::move(in[z++]);}
        else{src=fromB?1:0;out[o++]=std::move(in[ab]);if(fromB)++y;else++x;have=false;}
        if(src==last)++streak;else{last=src;streak=1;}
        if(streak<TRIGGER)continue;
        std::size_t end=0;
        if(src==2 && z<d && (x<b||y<c)){
            if(!have){if(x==b)fromB=true;else if(y==c)fromB=false;else fromB=less(in[y],in[x]);have=true;}
            const T&q=in[fromB?y:x];
            end=static_cast<std::size_t>(std::lower_bound(in.begin()+static_cast<std::ptrdiff_t>(z),in.begin()+static_cast<std::ptrdiff_t>(d),q,less)-in.begin());
            if(end>=z+MINSPAN){while(z<end)out[o++]=std::move(in[z++]);}
        }else if(src==0 && x<b){
            std::size_t ex=b;
            if(y<c){auto it=std::upper_bound(in.begin()+static_cast<std::ptrdiff_t>(x),in.begin()+static_cast<std::ptrdiff_t>(ex),in[y],less);ex=std::min(ex,static_cast<std::size_t>(it-in.begin()));}
            if(z<d){auto it=std::upper_bound(in.begin()+static_cast<std::ptrdiff_t>(x),in.begin()+static_cast<std::ptrdiff_t>(ex),in[z],less);ex=std::min(ex,static_cast<std::size_t>(it-in.begin()));}
            if(ex>=x+MINSPAN){while(x<ex)out[o++]=std::move(in[x++]);have=false;}
        }else if(src==1 && y<c){
            std::size_t ey=c;
            if(x<b){auto it=std::lower_bound(in.begin()+static_cast<std::ptrdiff_t>(y),in.begin()+static_cast<std::ptrdiff_t>(ey),in[x],less);ey=std::min(ey,static_cast<std::size_t>(it-in.begin()));}
            if(z<d){auto it=std::upper_bound(in.begin()+static_cast<std::ptrdiff_t>(y),in.begin()+static_cast<std::ptrdiff_t>(ey),in[z],less);ey=std::min(ey,static_cast<std::size_t>(it-in.begin()));}
            if(ey>=y+MINSPAN){while(y<ey)out[o++]=std::move(in[y++]);have=false;}
        }
        streak=0;last=-1;
    }
    while(x<b||y<c){if(x==b)out[o++]=std::move(in[y++]);else if(y==c)out[o++]=std::move(in[x++]);else if(less(in[y],in[x]))out[o++]=std::move(in[y++]);else out[o++]=std::move(in[x++]);}
    while(z<d)out[o++]=std::move(in[z++]);
}

template <class T, class Less>
inline bool e656DisjointRange(const std::vector<T>& v,
                              std::size_t a,std::size_t b,
                              std::size_t c,std::size_t d,Less less){
    if(a==b||c==d) return true;
    return less(v[b-1],v[c]) || less(v[d-1],v[a]);
}

template <class T, class Less>
inline bool e656AdmitTriple(const std::vector<T>& v,
                            std::size_t a,std::size_t b,
                            std::size_t c,std::size_t d,Less less,
                            bool highEntropyBias){
    // High-entropy is the mature comparator/pile-density signal already used by
    // E601 for its branchless merge. Otherwise require local evidence: strong
    // equality or three mutually non-overlapping sorted ranges. The latter is
    // a generic shuffled-run/block geometry, not a benchmark-family label.
    if(highEntropyBias) return true;
    if(e656EqualityScore(v,a,b,c,d,less)>=6) return true;
    if(d-a<512) return false;
    return e656DisjointRange(v,a,b,b,c,less) &&
           e656DisjointRange(v,b,c,c,d,less) &&
           e656DisjointRange(v,a,b,c,d,less);
}

template <class T, class Less>
void mergeRunsE656LocalTripleFirst(std::vector<T>& src,std::vector<T>& dst,
                                   std::vector<std::size_t>& ends,Less less,
                                   bool branchlessRandomMerge,bool bidirectionalBranchlessMerge=true){
    if(ends.size()<=1) return;
    std::vector<std::size_t> starts; starts.reserve(ends.size()+1); starts.push_back(0);
    for(auto e:ends) starts.push_back(e);
    if(starts.size()>2){std::size_t w=1;for(std::size_t r=1;r+1<starts.size();++r){auto q=starts[r];if(less(src[q],src[q-1]))starts[w++]=q;}starts[w++]=starts.back();starts.resize(w);}
    const std::size_t rc=starts.size()-1;
    // E661 correctness repair: after compacting already-ordered boundaries,
    // rc<=1 means src itself is already one globally sorted run.  The old E656
    // path swapped in dst here even though dst still holds stale/original data.
    if(rc<=1) return;
    std::vector<std::size_t> next; next.reserve((rc+1)/2);
    std::size_t r=0;
    while(r<rc){
        if(r+2<rc){
            const auto a=starts[r],b=starts[r+1],c=starts[r+2],d=starts[r+3];
            if(e656AdmitTriple(src,a,b,c,d,less,branchlessRandomMerge)){
                if(e656PreferGallop(src,a,b,c,d,less)) e656GallopTriple(src,dst,a,b,c,d,less);
                else e656ScalarTriple(src,dst,a,b,c,d,less);
                next.push_back(d); r+=3; continue;
            }
        }
        if(r+1<rc){
            const auto a=starts[r],b=starts[r+1],d=starts[r+2];
            if(branchlessRandomMerge){
                if(bidirectionalBranchlessMerge) mergeTwoAdjacentRunsToDestBranchlessBidirectional(src,dst,a,b,d,less);
                else mergeTwoAdjacentRunsToDestBranchless(src,dst,a,b,d,less);
            }else mergeTwoAdjacentRunsToDest(src,dst,a,b,d,less,7);
            next.push_back(d); r+=2;
        }else{
            const auto a=starts[r],d=starts[r+1];
            std::move(src.begin()+static_cast<std::ptrdiff_t>(a),src.begin()+static_cast<std::ptrdiff_t>(d),dst.begin()+static_cast<std::ptrdiff_t>(a));
            next.push_back(d); ++r;
        }
    }
    if(next.size()>1) mergeRunsAdjacentPairsEnds(dst,src,next,less,branchlessRandomMerge,bidirectionalBranchlessMerge);
    src.swap(dst);
}
...
ReconstructedRuns simulateAndReconstructRunsToTemp(
    ...
    std::vector<std::size_t> runStart =
        reconstructTaggedBlueprintToTemp_WithSplitCounts(
            ...
            std::move(sim.detachedAscCounts),
            std::move(sim.detachedDescCounts),
            tmp,
            false,
            true
        );
...
#include <jessesort/detail/backends/high_entropy/backend.inc>

#include <jessesort/detail/backends/high_entropy/vector_routes.inc>
...
bool tryLongAscendingNaturalRunDirect(std::vector<T>& arr, Less less,
    ...
    // Cheap real precondition from the repeated-pattern proof: the first
    // element of run 1 must equal the first element of run 0. Reject before
    // entering the cold full-pattern verifier when it cannot possibly match.
    bool repeatedPatternCandidate = false;
    if constexpr (specializedIntegralEligible<T, Less>) {
        repeatedPatternCandidate = (arr[runs[1].begin] == arr[0]);
    } else {
        repeatedPatternCandidate = !less(arr[runs[1].begin], arr[0]) &&
                                   !less(arr[0], arr[runs[1].begin]);
    }
    if (repeatedPatternCandidate &&
...
void sortImplCoreWithCheckpoint(std::vector<T>& arr, Less less, bool enableNaturalRunRoute,
    ...
    static_assert(std::is_copy_constructible_v<T> && std::is_copy_assignable_v<T>,
                  "jessesort::simulated_direct_dense_ghost::sort requires copyable values because pile tails are stored by value");
    static_assert(std::is_move_constructible_v<T> && std::is_move_assignable_v<T>,
                  "jessesort::simulated_direct_dense_ghost::sort requires movable values for merging");
    ...
    T9PressureState t9Pressure;
    g_t9PressureState = &t9Pressure;
    SimulatedInsertionResult<T> sim =
    ...
    g_t9PressureState = nullptr;

    if (sim.checkpointHandled) return;

    if (t9Pressure.tripIndex != std::numeric_limits<std::size_t>::max() &&
        t9Pressure.tripIndex < arr.size()) {
        const std::size_t prefixLen = t9Pressure.tripIndex;
        sim.blueprint.resize(prefixLen);
        std::vector<T> prefixSource(arr.begin(), arr.begin() + static_cast<std::ptrdiff_t>(prefixLen));
        std::vector<T> prefixTmp;
        auto runStart = reconstructTaggedBlueprintNormalizedForSimulated(
            prefixSource, std::move(sim.blueprint), std::move(sim.ascCounts),
            std::move(sim.descCounts), std::move(sim.detachedAscCounts),
            std::move(sim.detachedDescCounts), prefixTmp, false, true, true, true);

        if constexpr (specializedIntegralEligible<T, Less> && std::is_default_constructible_v<T>) {
            highEntropyQuickSort(arr.data() + static_cast<std::ptrdiff_t>(prefixLen),
                                 arr.size() - prefixLen,
                                 2 * static_cast<int>(std::bit_width(arr.size() - prefixLen)),
                                 less, false, T{}, false, true, nullptr, false);
        } else {
            std::sort(arr.begin() + static_cast<std::ptrdiff_t>(prefixLen), arr.end(), less);
        }
        std::vector<T> combined;
        if constexpr (std::is_default_constructible_v<T>) combined.resize(arr.size());
        else combined.assign(arr.begin(), arr.end());
        std::move(prefixTmp.begin(), prefixTmp.end(), combined.begin());
        std::move(arr.begin() + static_cast<std::ptrdiff_t>(prefixLen), arr.end(),
                  combined.begin() + static_cast<std::ptrdiff_t>(prefixLen));
        if (runStart.empty() || runStart.back() != prefixLen) runStart.push_back(prefixLen);
        runStart.push_back(arr.size());
        mergeRunsFromTmpToArr(combined, arr, std::move(runStart), less);
        return;
    }
    ...
    // E115: physical/simulated/simulated-inplace now share the ends-based adj-pair merge driver.
    ...
    // no longer earns its selector complexity. Use adj-pair scheduling.
    // E375: compact, size-imbalanced reconstructed run sets benefit from a
    // cheap sliding best-2-of-3 alphabetic tree. Stride 3 preserves most of
    // the adjacent driver's locality while avoiding repeatedly merging a
    // dominant run. Uniform compact sets and large run sets stay adjacent.
    const std::size_t reconstructedRuns = runStart.size() - 1;
    std::size_t maxReconstructedRun = 0;
    for (std::size_t r = 0; r < reconstructedRuns; ++r)
        maxReconstructedRun = std::max(
            maxReconstructedRun, runStart[r + 1] - runStart[r]);
    const bool useSlidingTwoOfThree =
        arr.size() >= 32768 && reconstructedRuns >= 8 && reconstructedRuns <= 64 &&
        static_cast<unsigned long long>(maxReconstructedRun) * reconstructedRuns * 2 >=
            static_cast<unsigned long long>(arr.size()) * 3;
    if (useSlidingTwoOfThree) {
        // E601: only compact E375-selected geometries. Larger run sets retain
        // the mature sliding scheduler (notably Random%25's usual geometry).
        if (reconstructedRuns <= 32) {
            detail::mergeRunsE601InternalWriteTree(
                tmp, arr, std::move(runStart), less);
        } else {
            detail::mergeRunsE375Tree(
                tmp, arr, std::move(runStart), less,
                detail::E375Schedule::sliding_stride3);
        }
    } else {
        std::vector<std::size_t> ends(runStart.begin() + 1, runStart.end());
        mergeRunsE656LocalTripleFirst(
...
bool tryStructuralPreRoutesE271(std::vector<T>& arr, Less less) {
    ...
    if (prefixInversions == 0) {
        ...
        if (!g_e725_disable_sparse_disorder && trySparseDistributedDisorderBlocks(arr, less, 0)) return true;
    } else if (prefixInversions <= 2) {
        if (!g_e725_disable_sparse_disorder && trySparseDistributedDisorderBlocks(
...
// E601: exact alphabetic DP for internal merge writes only.  For an interval
// [i,j], each possible root split pays the interval length once plus the two
// child costs.  Unlike E593, buffer parity is not part of the DP state; after
// the tree is fixed we reuse the mature leaf-copy parity chooser.
template <typename T, typename Less>
#if defined(__GNUC__) || defined(__clang__)
__attribute__((noinline, cold))
#endif
    ...
    std::vector<std::size_t> runStart, Less less)
{
    ...
    // Match the mature E375 path's ordered-boundary elimination exactly.
    if (runStart.size() > 2) {
        std::size_t write = 1;
        for (std::size_t r = 1; r + 1 < runStart.size(); ++r) {
            const auto b = runStart[r];
    ...
    constexpr unsigned long long INF = ~0ULL / 4ULL;
    unsigned long long cost[32][32];
    unsigned char split[32][32];
    unsigned long long prefix[33];
    prefix[0] = 0;
    for (std::size_t i = 0; i < rc; ++i) {
        prefix[i + 1] = prefix[i] + (runStart[i + 1] - runStart[i]);
        cost[i][i] = 0;
        split[i][i] = static_cast<unsigned char>(i);
    }

    for (std::size_t len = 2; len <= rc; ++len) {
        for (std::size_t i = 0; i + len <= rc; ++i) {
            const std::size_t j = i + len - 1;
            const unsigned long long interval = prefix[j + 1] - prefix[i];
            unsigned long long best = INF;
            std::size_t bestk = i;
            for (std::size_t k = i; k < j; ++k) {
                const unsigned long long c = interval + cost[i][k] + cost[k + 1][j];
                if (c < best) { best = c; bestk = k; }
            }
            cost[i][j] = best;
            split[i][j] = static_cast<unsigned char>(bestk);
        }
    }

    std::vector<PowerSortMergeNode> nodes;
    nodes.reserve(rc * 2 - 1);
    auto build = [&](auto&& self, std::size_t i, std::size_t j) -> std::size_t {
        if (i == j) {
            const auto idx = nodes.size();
            nodes.push_back(PowerSortMergeNode{
                runStart[i], runStart[i], runStart[i + 1], 0, 0, true});
            return idx;
        }
        const std::size_t k = split[i][j];
        const auto left = self(self, i, k);
        const auto right = self(self, k + 1, j);
        return makePowerSortMergedNode(nodes, left, right);
    };

    const auto root = build(build, 0, rc - 1);
    const auto copyToArr = powerSortLeafCopyCost(nodes, root, true);
    const auto copyToTmp = powerSortLeafCopyCost(nodes, root, false);
    const bool outputToArr = copyToArr <= copyToTmp;
    materializePowerSortTree(nodes, root, outputToArr, tmp, arr, less);
    if (!outputToArr) arr.swap(tmp);
}
