#ifndef JESSESORT_E229_JESSESORT_INDEXED_PROBE_ROUTED_ADJACENT_ADAPTIVE_BUFFERED_MOVE_ONLY_H
#define JESSESORT_E229_JESSESORT_INDEXED_PROBE_ROUTED_ADJACENT_ADAPTIVE_BUFFERED_MOVE_ONLY_H
...
inline void insertOne(const std::vector<T>& a, std::size_t i, bool desc, IndexSimResult<T>& r,
    ...
    if(desc){ auto p=findDesc(a,r.descTails,dh,i,less,useHint); if(p<r.descTails.size()){r.descTails[p]=i;++r.descCounts[p];} else {r.descTails.push_back(i);r.descCounts.push_back(1);} dh=p; r.blueprint[i]=jessesort::simulated_legacy::makeDescTag((uint32_t)p); }
    else { auto p=findAsc(a,r.ascTails,ah,i,less,useHint); if(p<r.ascTails.size()){r.ascTails[p]=i;++r.ascCounts[p];} else {r.ascTails.push_back(i);r.ascCounts.push_back(1);} ah=p; r.blueprint[i]=jessesort::simulated_legacy::makeAscTag((uint32_t)p); }
}
...
IndexSimResult<T> simulate(const std::vector<T>& a, Less less, bool natural=true, bool forceNoSpecial=false) {
    ...
    auto reserve=jessesort::simulated_legacy::estimatePileReserve(n); r.ascCounts.reserve(reserve);r.descCounts.reserve(reserve);r.ascTails.reserve(reserve);r.descTails.reserve(reserve);
    std::size_t ah=0,dh=0; bool dm=false; std::size_t ps=1;
    bool confirmed = pd!=Dir::Unknown && pe<MinPrefix && jessesort::simulated_legacy::confirmedShortSameDirectionPrefix(a,pe,pd==Dir::Desc,less,MinPrefix);
    bool materialize=pd!=Dir::Unknown && (pe>=MinPrefix || confirmed);
    if(materialize){ps=pe;if(pd==Dir::Asc){r.ascTails.push_back(pe-1);r.ascCounts.push_back(pe);std::fill_n(r.blueprint.begin(),pe,jessesort::simulated_legacy::makeAscTag(0));dm=false;}else{r.descTails.push_back(pe-1);r.descCounts.push_back(pe);std::fill_n(r.blueprint.begin(),pe,jessesort::simulated_legacy::makeDescTag(0));dm=true;}}
    else {if(pd==Dir::Desc){r.descTails.push_back(0);r.descCounts.push_back(1);r.blueprint[0]=jessesort::simulated_legacy::makeDescTag(0);dm=true;}else{r.ascTails.push_back(0);r.ascCounts.push_back(1);r.blueprint[0]=jessesort::simulated_legacy::makeAscTag(0);dm=false;}}
    ...
    if(nat && ps<n){ std::size_t i=ps; bool op=override;
      auto one=[&](std::size_t j){ if(op){dm=od;op=false;} else if(less(a[j-1],a[j]))dm=false; else if(less(a[j],a[j-1]))dm=true; else {if(dm){++r.descCounts[dh];r.blueprint[j]=jessesort::simulated_legacy::makeDescTag((uint32_t)dh);}else{++r.ascCounts[ah];r.blueprint[j]=jessesort::simulated_legacy::makeAscTag((uint32_t)ah);}return;} insertOne(a,j,dm,r,ah,dh,less,true); };
      while(i<n){bool asc,desc;if(op){asc=!od;desc=od;}else{asc=less(a[i-1],a[i]);desc=!asc&&less(a[i],a[i-1]);} if(!asc&&!desc){one(i++);continue;} std::size_t e=i+1;if(asc){while(e<n&&less(a[e-1],a[e]))++e;}else{while(e<n&&less(a[e],a[e-1]))++e;} auto len=e-i;bool bat=false;if(len>=8){if(asc){auto p1=findAsc(a,r.ascTails,ah,i,less,false),p2=findAsc(a,r.ascTails,ah,e-1,less,false);if(p1==p2){if(p1<r.ascTails.size()){r.ascTails[p1]=e-1;r.ascCounts[p1]+=len;}else{r.ascTails.push_back(e-1);r.ascCounts.push_back(len);}std::fill(r.blueprint.begin()+i,r.blueprint.begin()+e,jessesort::simulated_legacy::makeAscTag((uint32_t)p1));ah=p1;dm=false;op=false;bat=true;}}else{auto p1=findDesc(a,r.descTails,dh,i,less,false),p2=findDesc(a,r.descTails,dh,e-1,less,false);if(p1==p2){if(p1<r.descTails.size()){r.descTails[p1]=e-1;r.descCounts[p1]+=len;}else{r.descTails.push_back(e-1);r.descCounts.push_back(len);}std::fill(r.blueprint.begin()+i,r.blueprint.begin()+e,jessesort::simulated_legacy::makeDescTag((uint32_t)p1));dh=p1;dm=true;op=false;bat=true;}}}if(!bat)for(std::size_t j=i;j<e;++j)one(j);i=e;} return r; }
    auto process=[&](std::size_t i,bool useHint){if(override){dm=od;override=false;}else if(less(a[i-1],a[i]))dm=false;else if(less(a[i],a[i-1]))dm=true;else{if(dm){++r.descCounts[dh];r.blueprint[i]=jessesort::simulated_legacy::makeDescTag((uint32_t)dh);}else{++r.ascCounts[ah];r.blueprint[i]=jessesort::simulated_legacy::makeAscTag((uint32_t)ah);}return true;} std::size_t old=dm?dh:ah; bool had=dm?!r.descTails.empty():!r.ascTails.empty();insertOne(a,i,dm,r,ah,dh,less,useHint);return had&&old==(dm?dh:ah);};
    ...
    for(;i<n;++i){ if(valley){ bool desc=less(a[i],a[i-1]),asc=!desc&&less(a[i-1],a[i]); if(desc&&i+1<n&&less(a[i],a[i+1])){bool create=r.descTails.empty()||less(a[r.descTails.back()],a[i]);bool can=!r.ascTails.empty()&&!less(a[i],a[r.ascTails.back()]);if(create&&can){desc=false;asc=true;}} if(asc)dm=false;else if(desc)dm=true;else{if(dm){++r.descCounts[dh];r.blueprint[i]=jessesort::simulated_legacy::makeDescTag((uint32_t)dh);}else{++r.ascCounts[ah];r.blueprint[i]=jessesort::simulated_legacy::makeAscTag((uint32_t)ah);}continue;} insertOne(a,i,dm,r,ah,dh,less,true); }
...
std::vector<std::size_t> reconstructMoveOnly(
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        const std::size_t local = static_cast<std::size_t>(jessesort::simulated_legacy::localPileId(tag));
        const bool desc = jessesort::simulated_legacy::isDescTag(tag);
