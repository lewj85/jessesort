#ifndef JESSESORT_DETAIL_PHASE_MAP_RANGE_CORE_H
#define JESSESORT_DETAIL_PHASE_MAP_RANGE_CORE_H
...
template <class T, class Less>
PhaseMap mapX2Cap4k(const std::vector<T>& v, Less less) {
    PhaseMap m; const std::size_t n=v.size(); if(n==0) return m;
    auto obs=[&](std::size_t p){++m.probes;return stateOf(observe32(v,p,less));};
    std::size_t pos=0,gap=1024,lastStable=0; RouteState cur=obs(0);
    m.bounds.push_back(0);m.states.push_back(cur);
    while(pos+32<n){
        ...
        if(s!=cur){
            ...
            pos=nxt;gap=1024;
        }else{lastStable=nxt;pos=nxt;gap=std::min<std::size_t>(4096,gap*2);}
        if(pos>=n-32)break;
    }
    m.bounds.push_back(n);return m;
}
...
template <class T, class Less>
PhaseMap mapX2LeadStructuredCap3k(const std::vector<T>& v, Less less) {
    PhaseMap m; const std::size_t n=v.size(); if(n==0) return m;
    auto obs=[&](std::size_t p){++m.probes;return stateOf(observe32(v,p,less));};
    std::size_t pos=0,gap=1024,lastStable=0; RouteState cur=obs(0);
    const bool structuredLeading = cur == RouteState::Structured;
    m.bounds.push_back(0);m.states.push_back(cur);
    while(pos+32<n){
        const std::size_t nxt=std::min(n-32,pos+gap); if(nxt<=pos)break;
        const RouteState s=obs(nxt);
        if(s!=cur){
            ...
            lastStable=nxt;pos=nxt;
            const std::size_t cap = (structuredLeading && m.states.size()==1) ? 3072 : 4096;
            gap=std::min<std::size_t>(cap,gap*2);
        }
        if(pos>=n-32)break;
    }
    m.bounds.push_back(n);return m;
...
void mergeRuns(std::vector<T>& a,const std::vector<std::size_t>& initial,Less less){
    ...
    // First pass appends directly into an empty buffer. E250 copied the entire
    // array into tmp before this pass; mapped execution does not need that copy.
    {
        ...
        for(std::size_t r=0;r<runs;r+=2){
            ...
            if(r+1<runs){
                ...
                std::merge(std::make_move_iterator(a.begin()+static_cast<std::ptrdiff_t>(b)),
                           std::make_move_iterator(a.begin()+static_cast<std::ptrdiff_t>(mid)),
                           std::make_move_iterator(a.begin()+static_cast<std::ptrdiff_t>(mid)),
                           std::make_move_iterator(a.begin()+static_cast<std::ptrdiff_t>(e)),
                           std::back_inserter(tmp),less);
                next.push_back(e);
            }else{
                tmp.insert(tmp.end(),
                           std::make_move_iterator(a.begin()+static_cast<std::ptrdiff_t>(b)),
                           std::make_move_iterator(a.begin()+static_cast<std::ptrdiff_t>(mid)));
                next.push_back(mid);
            }
        }
    ...
    while(starts.size()>2){
        ...
        for(std::size_t r=0;r<runs;r+=2){
            const std::size_t b=starts[r],mid=starts[r+1];
            if(r+1<runs){const std::size_t e=starts[r+2];std::merge(src.begin()+static_cast<std::ptrdiff_t>(b),src.begin()+static_cast<std::ptrdiff_t>(mid),src.begin()+static_cast<std::ptrdiff_t>(mid),src.begin()+static_cast<std::ptrdiff_t>(e),dst.begin()+static_cast<std::ptrdiff_t>(b),less);next.push_back(e);}
            else{std::move(src.begin()+static_cast<std::ptrdiff_t>(b),src.begin()+static_cast<std::ptrdiff_t>(mid),dst.begin()+static_cast<std::ptrdiff_t>(b));next.push_back(mid);}
        }
