...
void sort(std::vector<T>& a,Less less=Less{}){
 ...
 if constexpr (std::is_copy_constructible_v<T> && std::is_copy_assignable_v<T>) {
   starts=jessesort::simulated_direct_merge::reconstructTaggedBlueprintNormalizedForV2(a,std::move(s.blueprint),std::move(s.ascCounts),std::move(s.descCounts),tmp,false,true,true,true);
 } else {
