...
namespace jessesort::linked_v8_legacy {
...
struct SimulatedInsertionResult {
    ...
    // V8/E191 native linked blueprint. During Patience insertion blueprint[i]
    ...
    // same 4-byte/element metadata footprint as V2 instead of storing tags+links.
...
SimulatedInsertionResult<T> simulatePatienceInsertionBlueprintImpl(
    ...
    const bool confirmedShortSameDirectionPrefix =
        ...
        jessesort::linked_v8_legacy::confirmedShortSameDirectionPrefix(
...
void sortImplCore(std::vector<T>& arr, Less less, bool enableNaturalRunRoute,
    ...
    static_assert(std::is_copy_constructible_v<T> && std::is_copy_assignable_v<T>,
                  "jessesort::linked_v8_legacy::sort requires copyable values because pile tails are stored by value");
    static_assert(std::is_move_constructible_v<T> && std::is_move_assignable_v<T>,
                  "jessesort::linked_v8_legacy::sort requires movable values for merging");
    ...
    if (linkedRoutingMode == 8) {
        ...
        if (useLinkedReconstruction) {
            ...
            runStart = jessesort::simulated_legacy::reconstructTaggedBlueprintNormalizedForV2(
...
void sortLooseLinked(std::vector<T>& arr, Less less = Less{}, bool* linkedUsedOut = nullptr) {
    ...
    // all production Patience candidates enter V8. Inner routing uses links for
...
void sort(std::vector<T>& arr, Less less = Less{}) {
    // E201 retained V8 baseline: remain linked whenever Patience reconstruction
    ...
    // in-place element links. No V2/tag reconstruction fallback is used here.
