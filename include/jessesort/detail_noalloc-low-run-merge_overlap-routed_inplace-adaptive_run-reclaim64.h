#ifndef JESSESORT_DETAIL_NOALLOC_LOW_RUN_MERGE_OVERLAP_ROUTED_INPLACE_ADAPTIVE_RUN_RECLAIM64_H
#define JESSESORT_DETAIL_NOALLOC_LOW_RUN_MERGE_OVERLAP_ROUTED_INPLACE_ADAPTIVE_RUN_RECLAIM64_H

#include <jessesort/detail_noalloc_bounded_common.h>
...
namespace jessesort::allocation_free_low_run {

#if defined(__GNUC__) || defined(__clang__)
#define JESSESORT_NOALLOC_LOW_RUN_NOINLINE __attribute__((noinline, cold))
#else
#define JESSESORT_NOALLOC_LOW_RUN_NOINLINE
#endif

// noalloc-low-run identity (E216): allocation-free bounded JesseSort with a shared
...
// noalloc-bounded's allocation-free fallback path.
...
inline constexpr std::size_t kMaxRuns = allocation_free_bounded::kMaxRuns;
...
void mergeNaturalRunsNoAlloc(std::vector<T>& a,
                             std::array<allocation_free_bounded::RunDesc, kMaxRuns>& runs,
                             std::size_t runCount,
                             Less less) {
...
bool repeatedOverlapGeometry(const std::vector<T>& a,
                             const std::array<allocation_free_bounded::RunDesc, kMaxRuns>& runs,
                             std::size_t runCount,
                             Less less) {
    ...
    for (std::size_t r = 0; r + 1 < runCount; ++r) {
        ...
        if (less(rmin, lmax) && less(lmin, rmax)) ++overlapCount;
    }

    const std::size_t boundaries = runCount - 1;
    if (overlapCount == boundaries) return true;

    // E338: recursive in-place merging is also a poor trade when short runs
    // overlap at nearly every boundary. Preserve the old unconditional
    // all-overlap bailout, but reject dense near-overlap only when the runs
    // are short enough that repeated rotations dominate. The 512-element
    // boundary is deliberately coarse; 384/512/640 all behaved similarly.
    constexpr std::size_t kMinAverageRunForNearOverlapMerge = 512;
    if (a.size() >= runCount * kMinAverageRunForNearOverlapMerge) return false;
    return overlapCount * 10 >= boundaries * 9;
}
...
First65RunProfile profileFirst65Runs(const std::vector<T>& a, Less less) {
    ...
    allocation_free_bounded::RunDesc previous{};
    ...
    while (i < a.size() && rawRunCount < kMaxRuns + 1) {
        ...
        const allocation_free_bounded::RunDesc current{begin, i, descending};
        if (havePrevious) {
...
template <class T, class Less>
void normalizeRunNoAlloc(std::vector<T>& a, allocation_free_bounded::RunDesc& run, Less) {
...
template <class T, class Less>
JESSESORT_NOALLOC_LOW_RUN_NOINLINE void handleRunCapacityOverflow(
    std::vector<T>& a,
    std::array<allocation_free_bounded::RunDesc, kMaxRuns>& runs,
    std::size_t& runCount,
    Less less,
...
void sortWithMetrics(std::vector<T>& a, Less less = Less{}, Metrics* metrics = nullptr) {
    static_assert(std::is_move_constructible_v<T> && std::is_move_assignable_v<T>,
                  "noalloc-low-run requires movable values");
    if (a.size() < 2) return;
    ...
    if (classified != ClassifyResult::Accept) { allocation_free_bounded::fallbackNoAlloc(a, less); return; }
    ...
    if (repeatedOverlapGeometry(a, runs, runCount, less)) {
        ...
        allocation_free_bounded::fallbackNoAlloc(a, less); return;
    }
...
#undef JESSESORT_NOALLOC_LOW_RUN_NOINLINE
