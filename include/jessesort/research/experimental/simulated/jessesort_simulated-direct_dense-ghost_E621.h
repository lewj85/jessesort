#ifndef JESSESORT_SIMULATED_DIRECT_DENSE_GHOST_E621_H
#define JESSESORT_SIMULATED_DIRECT_DENSE_GHOST_E621_H
...
namespace jessesort::experimental::simulated_direct_dense_ghost_e621 {
...
inline std::uint64_t e621NarrowReconstructCount = 0;
inline std::uint64_t e621WideReconstructFallbackCount = 0;
...
std::vector<std::size_t> reconstructTaggedBlueprintNormalizedForSimulated(
    ...
    // E621: insertion remains exact E601 with 32-bit packed tags. Only after
    // insertion and final run geometry are known do we normalize to compact
    // 16-bit run IDs for reconstruction. This deliberately isolates the
    // reconstruction/tag-traffic question from E615/E617 narrow insertion.
    // If the final run alphabet cannot be represented, preserve E601 exactly.
    if (numRuns <= static_cast<std::size_t>(UINT16_MAX)) {
        ++e621NarrowReconstructCount;
        // Compact two normalized 16-bit run IDs into each existing 32-bit
        // blueprint word. Forward compaction is safe: destination word i/2
        // is never ahead of source word i, so no unread source tag is clobbered.
        // This removes E620's second allocation/copy/free while causing the
        // scatter to touch only the first ceil(n/2) blueprint words.
        for (std::size_t i = 0; i < n; ++i) {
            const std::size_t r = decodeRun(blueprint[i]);
            assert(r <= static_cast<std::size_t>(UINT16_MAX));
            const std::size_t w = i >> 1;
            if ((i & 1u) == 0u)
                blueprint[w] = static_cast<std::uint32_t>(r);
            else
                blueprint[w] = (blueprint[w] & 0x0000ffffu) |
                    (static_cast<std::uint32_t>(r) << 16);
        }
        const auto packedRun = [&](std::size_t i) -> std::uint16_t {
            const std::uint32_t word = blueprint[i >> 1];
            return static_cast<std::uint16_t>((i & 1u) ? (word >> 16) : word);
        };

        if (useBulkTagSpans) {
            std::size_t i = 0;
            while (i < n) {
                const std::uint16_t tag = packedRun(i);
                const std::size_t r = static_cast<std::size_t>(tag);
                std::size_t j = i + 1;
                while (j < n && packedRun(j) == tag) ++j;
            ...
            for (std::size_t i = 0; i < n; ++i) {
                const std::size_t r = static_cast<std::size_t>(packedRun(i));
                const std::size_t pos = cursor[r];
                if (deriveStepFromRunId) {
        ...
        ++e621WideReconstructFallbackCount;
