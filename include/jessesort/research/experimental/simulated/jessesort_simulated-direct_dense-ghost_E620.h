#ifndef JESSESORT_SIMULATED_DIRECT_DENSE_GHOST_E620_H
#define JESSESORT_SIMULATED_DIRECT_DENSE_GHOST_E620_H
...
namespace jessesort::experimental::simulated_direct_dense_ghost_e620 {
...
inline std::uint64_t e620NarrowReconstructCount = 0;
inline std::uint64_t e620WideReconstructFallbackCount = 0;
...
std::vector<std::size_t> reconstructTaggedBlueprintNormalizedForSimulated(
    ...
    // E620: insertion remains exact E601 with 32-bit packed tags. Only after
    ...
    if (numRuns <= static_cast<std::size_t>(UINT16_MAX)) {
        ++e620NarrowReconstructCount;
        std::vector<std::uint16_t> narrowBlueprint(n);
        for (std::size_t i = 0; i < n; ++i) {
            const std::size_t r = decodeRun(blueprint[i]);
            assert(r <= static_cast<std::size_t>(UINT16_MAX));
            narrowBlueprint[i] = static_cast<std::uint16_t>(r);
        }
        // Release the wide blueprint before the cache-sensitive scatter.
        std::vector<uint32_t>().swap(blueprint);

        if (useBulkTagSpans) {
            std::size_t i = 0;
            while (i < n) {
                const std::uint16_t tag = narrowBlueprint[i];
                const std::size_t r = static_cast<std::size_t>(tag);
                std::size_t j = i + 1;
                while (j < n && narrowBlueprint[j] == tag) ++j;
            ...
            for (std::size_t i = 0; i < n; ++i) {
                const std::size_t r = static_cast<std::size_t>(narrowBlueprint[i]);
                const std::size_t pos = cursor[r];
                if (deriveStepFromRunId) {
        ...
        ++e620WideReconstructFallbackCount;
