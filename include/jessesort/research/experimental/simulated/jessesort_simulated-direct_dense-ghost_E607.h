#ifndef JESSESORT_SIMULATED_DIRECT_DENSE_GHOST_E607_H
#define JESSESORT_SIMULATED_DIRECT_DENSE_GHOST_E607_H

#include <jessesort/detail/common/tiny_sort.h>
#include <jessesort/research/experimental/simulated/jessesort_simulated-direct_dense-ghost_E600.h>
#include <jessesort/research/experimental/simulated/jessesort_simulated-direct_dense-ghost_E604.h>
#include <vector>
#include <cstdint>
#include <cstring>
#include <cstddef>
...
namespace jessesort::experimental::simulated_direct_dense_ghost_e607 {
...
// =========================================================
// E604: 16-bit per-element packed blueprint tags (pile counters remain 32-bit)
...
using BlueprintTag = std::uint16_t;
static constexpr BlueprintTag DESC_BIT  = 0x8000u;
static constexpr BlueprintTag PILE_MASK = 0x3fffu;
static constexpr BlueprintTag OVERFLOW_TAG = 0xffffu;
static constexpr std::size_t NARROW_SWITCH_ID = 16384u;
struct NarrowBlueprintFallback {};
inline std::uint64_t narrowFallbackCount = 0;
...
static constexpr BlueprintTag DETACHED_BIT = 0x4000u;
static constexpr BlueprintTag DETACHED_ID_MASK = 0x3fffu;
inline BlueprintTag makeDetachedAscTag(uint32_t run) { assert(run <= DETACHED_ID_MASK); return static_cast<BlueprintTag>(DETACHED_BIT | run); }
inline BlueprintTag makeDetachedDescTag(uint32_t run) { assert(run <= DETACHED_ID_MASK); return static_cast<BlueprintTag>(DESC_BIT | DETACHED_BIT | run); }
inline bool isDetachedTag(BlueprintTag tag) { return (tag & DETACHED_BIT) != 0; }
inline uint32_t detachedRunId(BlueprintTag tag) { return tag & DETACHED_ID_MASK; }

inline BlueprintTag makeAscTag(uint32_t localPile) {
    assert(localPile <= PILE_MASK);
    return static_cast<BlueprintTag>(localPile);
}

inline BlueprintTag makeDescTag(uint32_t localPile) {
    assert(localPile <= PILE_MASK);
    return static_cast<BlueprintTag>(DESC_BIT | localPile);
}

inline bool isDescTag(BlueprintTag tag) {
...
inline uint32_t localPileId(BlueprintTag tag) {
...
struct SimulatedInsertionResult {
    std::vector<BlueprintTag> blueprint;
...
inline void simulateInsertValueDescendingPiles(
    ...
    if (pileIndex < tails.size()) {
        ...
        if (pileIndex == NARROW_SWITCH_ID) [[unlikely]] throw NarrowBlueprintFallback{};
...
inline void simulateInsertValueDescendingPilesSplit(
    ...
    std::size_t originalIndex, std::vector<BlueprintTag>& blueprint,
    ...
    else { if (pileIndex == NARROW_SWITCH_ID) [[unlikely]] throw NarrowBlueprintFallback{}; tails.push_back(value); descCounts.push_back(1); }
    lastPileIndex = pileIndex;
    blueprint[originalIndex] = makeDescTag(static_cast<uint32_t>(pileIndex));
}
...
inline void simulateInsertValueAscendingPilesSplit(
    ...
    else { if (pileIndex == NARROW_SWITCH_ID) [[unlikely]] throw NarrowBlueprintFallback{}; tails.push_back(value); ascCounts.push_back(1); }
    lastPileIndex = pileIndex;
    blueprint[originalIndex] = makeAscTag(static_cast<uint32_t>(pileIndex));
}
...
JESSESORT_CONTINUE_NOINLINE void continuePatienceInsertionCoherentCache(
    ...
    auto process = [&](const T& previous, const T& value, std::size_t i) {
        ...
        if (p < tails.size()) {
            ...
            if (p == NARROW_SWITCH_ID) [[unlikely]] throw NarrowBlueprintFallback{};
            ...
            if (asc) {
                ...
                if (firstPile == lastPile) {
                    if (firstPile < result.ascTails.size()) {
                        ...
                        if (firstPile == NARROW_SWITCH_ID) [[unlikely]] throw NarrowBlueprintFallback{};
                        result.ascTails.push_back(arr[end - 1]);
                        result.ascCounts.push_back(len);
                    }
                    const BlueprintTag tag = makeAscTag(static_cast<uint32_t>(firstPile));
                    ...
                    if (pieces >= 3 && len >= DENSE_GHOST_MIN_RUN_LENGTH) {
                        ...
                        if (ghost == NARROW_SWITCH_ID) [[unlikely]] throw NarrowBlueprintFallback{};
                        result.detachedAscCounts.push_back(ghostEnd - i);
                        const BlueprintTag tag = makeDetachedAscTag(static_cast<uint32_t>(ghost));
                        std::fill(result.blueprint.begin() + static_cast<std::ptrdiff_t>(i),
                ...
                if (firstPile == lastPile) {
                    ...
                    const BlueprintTag tag = makeDescTag(static_cast<uint32_t>(firstPile));
                    std::fill(result.blueprint.begin() + static_cast<std::ptrdiff_t>(i),
                              result.blueprint.begin() + static_cast<std::ptrdiff_t>(end), tag);
                    ...
                    if (pieces >= 3 && len >= DENSE_GHOST_MIN_RUN_LENGTH) {
                        ...
                        const BlueprintTag tag = makeDetachedDescTag(static_cast<uint32_t>(ghost));
                        std::fill(result.blueprint.begin() + static_cast<std::ptrdiff_t>(i),
                                  result.blueprint.begin() + static_cast<std::ptrdiff_t>(ghostEnd), tag);
                        end = ghostEnd;
                        descendingMode = true;
...
SimulatedInsertionResult<T> simulatePatienceInsertionBlueprintImpl(
    ...
    const bool confirmedShortSameDirectionPrefix =
        ...
        jessesort::experimental::simulated_direct_dense_ghost_e607::confirmedShortSameDirectionPrefix(
...
std::vector<std::size_t> reconstructTaggedBlueprintToTemp_WithSplitCounts(
    const std::vector<T>& arr,
    const std::vector<BlueprintTag>& blueprint,
    std::vector<std::size_t> ascCounts,
    std::vector<std::size_t> descCounts,
    std::vector<std::size_t> detachedAscCounts,
    std::vector<std::size_t> detachedDescCounts,
    ...
    for (std::size_t i = 0; i < n; ++i) {
        const BlueprintTag tag = blueprint[i];
        const bool desc = isDescTag(tag);
...
// E600: direct four-run tournament realization.  The reconstructed run set,
// routing, and merge semantics are unchanged; only the ordinary adjacent
// realization is widened from binary groups to groups of up to four.  A small
// fixed tournament needs at most two comparator updates per emitted element
// after initialization, matching the comparison depth of two binary levels
// while writing the element only once.
template <class T, class Less>
void mergeRunsFourWayEnds(std::vector<T>& src, std::vector<T>& dst,
                          std::vector<std::size_t>& ends, Less less,
                          bool branchlessRandomMerge,
                          bool bidirectionalBranchlessMerge = false) {
    if (ends.size() <= 1) return;
    (void)branchlessRandomMerge;
    (void)bidirectionalBranchlessMerge;

    bool sourceIsSrc = true;
    bool probeOrderedBoundaries = true;
    std::vector<std::size_t> next;
    next.reserve((ends.size() + 3) / 4);

    auto mergeGroup = [&](auto& in, auto& out,
                          std::size_t left, const std::size_t* groupEnds,
                          std::size_t count) {
        const std::size_t right = groupEnds[count - 1];
        if (count == 1) {
            std::move(in.begin() + static_cast<std::ptrdiff_t>(left),
                      in.begin() + static_cast<std::ptrdiff_t>(right),
                      out.begin() + static_cast<std::ptrdiff_t>(left));
            return;
        }

        // Preserve the cheap whole-span ordered fast path.
        bool globallyOrdered = true;
        std::size_t prev = left;
        for (std::size_t r = 0; r + 1 < count; ++r) {
            const std::size_t b = groupEnds[r];
            if (less(in[b], in[b - 1])) { globallyOrdered = false; break; }
            prev = b;
        }
        (void)prev;
        if (globallyOrdered) {
        ...
        std::size_t begin[4] = {left, right, right, right};
        std::size_t end[4]   = {right, right, right, right};
        begin[0] = left;
        end[0] = groupEnds[0];
        for (std::size_t r = 1; r < count; ++r) {
            begin[r] = groupEnds[r - 1];
            end[r] = groupEnds[r];
        }
        std::size_t pos[4] = {begin[0], begin[1], begin[2], begin[3]};

        auto better = [&](int a, int b) -> int {
            const bool aliveA = a >= 0 && pos[static_cast<std::size_t>(a)] < end[static_cast<std::size_t>(a)];
            const bool aliveB = b >= 0 && pos[static_cast<std::size_t>(b)] < end[static_cast<std::size_t>(b)];
            if (!aliveA) return aliveB ? b : -1;
            if (!aliveB) return a;
            // Left stream wins equivalence to preserve deterministic ordering.
            return less(in[pos[static_cast<std::size_t>(b)]],
                        in[pos[static_cast<std::size_t>(a)]]) ? b : a;
        };
        auto pairWinner = [&](int a, int b) -> int { return better(a, b); };

        int w01 = pairWinner(0, count > 1 ? 1 : -1);
        int w23 = pairWinner(count > 2 ? 2 : -1, count > 3 ? 3 : -1);
        int winner = better(w01, w23);
        std::size_t k = left;
        while (winner >= 0) {
            const std::size_t w = static_cast<std::size_t>(winner);
            out[k++] = std::move(in[pos[w]++]);
            if (winner < 2) w01 = pairWinner(0, count > 1 ? 1 : -1);
            else w23 = pairWinner(count > 2 ? 2 : -1, count > 3 ? 3 : -1);
            winner = better(w01, w23);
        }
    };
    ...
    while (ends.size() > 1) {
        ...
        if (probeOrderedBoundaries) {
            ...
            for (std::size_t r = 0; r + 1 < ends.size(); ++r) {
                const std::size_t boundary = ends[r];
                if (less(in[boundary], in[boundary - 1])) next.push_back(boundary);
            }
            next.push_back(ends.back());
            ends.swap(next);
            const std::size_t newRuns = ends.size();
        ...
        next.clear();
        std::size_t left = 0;
        for (std::size_t r = 0; r < ends.size(); r += 4) {
            const std::size_t count = std::min<std::size_t>(4, ends.size() - r);
            std::size_t groupEnds[4] = {0,0,0,0};
            for (std::size_t q = 0; q < count; ++q) groupEnds[q] = ends[r + q];
            mergeGroup(in, out, left, groupEnds, count);
            const std::size_t right = groupEnds[count - 1];
            next.push_back(right);
            left = right;
        }
...
bool tryLongAscendingNaturalRunDirect(std::vector<T>& arr, Less less,
    ...
    mergeRunsFourWayEnds(arr, tmp, ends, less, false, true);
    return true;
}
...
void sort(std::vector<T>& arr, Less less = Less{}) {
    // E607: a 14-bit local ID represents 0..16383, so any input of at most
    // 16,384 elements is provably representable without a dynamic guard.
    // Delegate that whole domain to E604 so its ID-creation path remains
    // branch-free.  The current implementation is serial with respect to ID
    // creation, therefore no speculative multi-worker reserve is required.
    if (arr.size() <= 16384u) {
        jessesort::experimental::simulated_direct_dense_ghost_e604::sort(arr, less);
        return;
    }
    ...
    } catch (const NarrowBlueprintFallback&) {
        ++narrowFallbackCount;
        jessesort::experimental::simulated_direct_dense_ghost_e600::sort(arr, less);
    }
}
