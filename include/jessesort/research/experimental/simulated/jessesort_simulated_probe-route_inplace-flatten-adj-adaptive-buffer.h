#ifndef JESSESORT_E229_JESSESORT_SIMULATED_PROBE_ROUTED_INPLACE_FLATTEN_ADJACENT_ADAPTIVE_BUFFERED_H
#define JESSESORT_E229_JESSESORT_SIMULATED_PROBE_ROUTED_INPLACE_FLATTEN_ADJACENT_ADAPTIVE_BUFFERED_H
...
namespace jessesort::simulated_inplace_flatten_legacy {
...
// ascending-game runs, matching physical/simulated.  The in-place permutation remains simulated-inplace's
    ...
    if (n <= static_cast<std::size_t>(std::numeric_limits<std::uint32_t>::max())) {
        ...
        for (std::size_t i = 0; i < n; ++i) {
            ...
            const std::size_t local = static_cast<std::size_t>(
                jessesort::simulated_legacy::localPileId(tag));
            const std::size_t destination =
                jessesort::simulated_legacy::isDescTag(tag)
                    ? --descCounts[local]
                    : ascCounts[local]++;
            blueprint[i] = static_cast<std::uint32_t>(destination);
        }

        // E318: wide values pay heavily for repeated value swaps.  For records
        // at least 192 bytes wide, invert the source->destination permutation
        // in place and resolve each cycle with one carried value instead.  The
        // 192-byte boundary is intentionally a coarse routing boundary: E318
        // validates the movement reduction strongly at the reconstruction
        // kernel while documenting that the exact whole-sort crossover remains
        // machine/layout sensitive in the middle-width region.
        if constexpr (sizeof(T) >= 192) {
            constexpr std::uint32_t mark = std::uint32_t{1} << 31;
            if (n < static_cast<std::size_t>(mark)) {
                for (std::uint32_t i = 0; i < static_cast<std::uint32_t>(n); ++i) {
                    if (blueprint[i] & mark) continue;
                    std::uint32_t prev = i;
                    std::uint32_t cur = blueprint[i];
                    while (cur != i) {
                        const std::uint32_t next = blueprint[cur];
                        blueprint[cur] = prev | mark;
                        prev = cur;
                        cur = next;
                    }
                    blueprint[i] = prev | mark;
                }
                constexpr std::uint32_t payload = ~mark;
                for (std::uint32_t i = 0; i < static_cast<std::uint32_t>(n); ++i) {
                    if ((blueprint[i] & payload) == i) continue;
                    T carried = std::move(arr[i]);
                    std::uint32_t cur = i;
                    while ((blueprint[cur] & payload) != i) {
                        const std::uint32_t src = blueprint[cur] & payload;
                        arr[cur] = std::move(arr[src]);
                        blueprint[cur] = cur | mark;
                        cur = src;
                    }
                    arr[cur] = std::move(carried);
                    blueprint[cur] = cur | mark;
                }
            } else {
                for (std::size_t i = 0; i < n; ++i) {
        ...
        for (std::size_t i = 0; i < n; ++i) {
            ...
            const std::size_t local = static_cast<std::size_t>(
                jessesort::simulated_legacy::localPileId(tag));
            destination[i] = jessesort::simulated_legacy::isDescTag(tag)
                ? --descCounts[local]
                : ascCounts[local]++;
        }
...
// E133/E156 audit: simulated-inplace deliberately shares simulated's specialized pre-Patience
// router. Keep the policy in one implementation so later simulated routing changes
// cannot silently fail to propagate to simulated-inplace.

// Public simulated-inplace entry point: simulated-style simulated insertion with in-place flattening.
...
void sortImpl(std::vector<T>& arr, Less less, bool bidirectionalBranchlessMerge, bool naturalRunRoute = false, bool enableSpecializedRoutes = false, bool enableCoherentValuePileCache = true) {
    static_assert(std::is_copy_constructible_v<T> && std::is_copy_assignable_v<T>,
                  "jessesort::simulated_inplace_flatten_legacy::sort requires copyable values because pile tails are stored by value");
    static_assert(std::is_move_constructible_v<T> && std::is_move_assignable_v<T>,
                  "jessesort::simulated_inplace_flatten_legacy::sort requires movable values for permutation swaps and merging");
    ...
    if constexpr (jessesort::simulated_legacy::specializedIntegralEligible<T, Less>) {
        if (enableSpecializedRoutes) {
            ...
            if (specialValuePrefixMayMix && jessesort::simulated_legacy::trySpecializedPrePatienceRoutes(arr, less)) {
                return;
            }
        }
    }
    ...
    bool branchlessRandomMerge = false;
    {
        ...
        // simulated-inplace's defining difference from simulated: resolve the exact same simulated
        // blueprint into the same run order by permutation-cycle flattening
        // directly in arr instead of simulated's streaming reconstruction into tmp.
    ...
    // E115: physical/simulated/simulated-inplace share the ends-based adj-pair merge driver.
    // The driver computes its own E045 gallop trigger from final run lengths.
    // E111: the reconciled physical/simulated/simulated-inplace revalidation no longer supports the
    // selective PowerSort path here. Keep the current adj-pair scheduler.
...
// Public simulated-inplace entry point. E087 retains E085's bidirectional branchless kernel
// only after a simulated-inplace-specific causal A/B; sortImpl remains available internally
