#ifndef JESSESORT_E588_SIMULATED_FROZEN_DIRECT_HIGHENTROPY_OVERFLOW_H
#define JESSESORT_E588_SIMULATED_FROZEN_DIRECT_HIGHENTROPY_OVERFLOW_H
...
namespace jessesort::experimental::simulated_frozen_direct_highentropy_overflow {
...
std::vector<std::size_t> reconstructFrozenBlueprintWithSingleOverflowRun(
    ...
    if (overflowCount != 0) {
        if (patienceSortOverflow) {
            ...
            // E588: the terminal overflow run is high-entropy residue from the
            // frozen patience decomposition. Sort it in place with JesseSort's
            // production ipnsort-inspired partition backend instead of handing
            // it to std::sort.
...
void sortImpl(std::vector<T>& arr, Less less, bool bidirectionalBranchlessMerge, bool naturalRunRoute = false, bool enableSpecializedRoutes = false, bool patienceSortOverflow = false, bool enableCoherentValuePileCache = true) {
    static_assert(std::is_copy_constructible_v<T> && std::is_copy_assignable_v<T>,
                  "jessesort::experimental::simulated_frozen_direct_highentropy_overflow::sort requires copyable values because pile tails are stored by value");
    static_assert(std::is_move_constructible_v<T> && std::is_move_assignable_v<T>,
                  "jessesort::experimental::simulated_frozen_direct_highentropy_overflow::sort requires movable values for merging");
