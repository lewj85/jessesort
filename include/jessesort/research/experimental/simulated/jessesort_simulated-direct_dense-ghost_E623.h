#ifndef JESSESORT_SIMULATED_DIRECT_DENSE_GHOST_E623_H
#define JESSESORT_SIMULATED_DIRECT_DENSE_GHOST_E623_H
...
namespace jessesort::simulated_direct_dense_ghost_e623 {
#ifdef JESSESORT_E618_RUNTIME_GATE
inline thread_local bool e618EnableExtendedScheduler = false;
#ifdef JESSESORT_E623_TELEMETRY
inline thread_local std::size_t e623Runs33to64 = 0;
inline thread_local std::size_t e623CurrentGateHits = 0;
inline thread_local unsigned long long e623ExactWrites = 0;
inline thread_local unsigned long long e623CurrentWrites = 0;
inline thread_local std::size_t e623Savings5 = 0;
inline thread_local std::size_t e623Savings10 = 0;
inline thread_local std::size_t e623Savings15 = 0;
inline thread_local std::size_t e623Savings20 = 0;
#endif
#endif
#ifdef JESSESORT_E618_TELEMETRY
inline thread_local std::size_t e618ExtendedSchedulerHits = 0;
inline thread_local std::size_t e618ExtendedSchedulerRuns = 0;
inline thread_local unsigned long long e618ExactInternalWrites = 0;
inline thread_local unsigned long long e618E375InternalWrites = 0;
#endif
...
SimulatedInsertionResult<T> simulatePatienceInsertionBlueprintImpl(
    ...
    const bool confirmedShortSameDirectionPrefix =
        ...
        jessesort::simulated_direct_dense_ghost_e601::confirmedShortSameDirectionPrefix(
...
#endif
void mergeRunsE618InternalWriteTree(
...
void sortImplCoreWithCheckpoint(std::vector<T>& arr, Less less, bool enableNaturalRunRoute,
    ...
    #ifdef JESSESORT_E623_TELEMETRY
    if (reconstructedRuns >= 33 && reconstructedRuns <= 64) {
        auto diagStart = runStart;
        if (diagStart.size() > 2) {
            std::size_t write = 1;
            for (std::size_t r = 1; r + 1 < diagStart.size(); ++r) {
                const auto b = diagStart[r];
                if (less(tmp[b], tmp[b - 1])) diagStart[write++] = b;
            }
            diagStart[write++] = tmp.size();
            diagStart.resize(write);
        }
        const std::size_t drc = diagStart.size() - 1;
        if (drc >= 2 && drc <= 64) {
            unsigned long long dp[64][64]{};
            unsigned long long pref[65]{};
            for (std::size_t r = 0; r < drc; ++r) pref[r+1] = pref[r] + (diagStart[r+1]-diagStart[r]);
            for (std::size_t len=2; len<=drc; ++len) for (std::size_t i=0;i+len<=drc;++i) {
                std::size_t j=i+len-1; unsigned long long best=~0ULL/4ULL; auto interval=pref[j+1]-pref[i];
                for (std::size_t k=i;k<j;++k) best=std::min(best, interval+dp[i][k]+dp[k+1][j]);
                dp[i][j]=best;
            }
            unsigned long long exact=dp[0][drc-1], current=0;
            std::vector<unsigned long long> active; active.reserve(drc);
            for(std::size_t r=0;r<drc;++r) active.push_back(diagStart[r+1]-diagStart[r]);
            if (useSlidingTwoOfThree) {
                while(active.size()>1){
                    if(active.size()==2){auto m=active[0]+active[1]; current+=m; active.assign(1,m); break;}
                    std::size_t i=0; bool any=false;
                    while(i+2<active.size()){auto ab=active[i]+active[i+1], bc=active[i+1]+active[i+2]; std::size_t pos=(ab<=bc)?i:i+1; auto m=active[pos]+active[pos+1]; current+=m; active[pos]=m; active.erase(active.begin()+static_cast<std::ptrdiff_t>(pos+1)); any=true; i=std::min(pos+std::size_t{3},active.size());}
                    if(!any){ // finish remaining runs with adjacent rounds
                        while(active.size()>1){std::vector<unsigned long long> next; next.reserve((active.size()+1)/2); for(std::size_t q=0;q<active.size();q+=2){if(q+1<active.size()){auto m=active[q]+active[q+1];current+=m;next.push_back(m);}else next.push_back(active[q]);} active.swap(next);} break;}
                }
            } else {
                while(active.size()>1){std::vector<unsigned long long> next; next.reserve((active.size()+1)/2); for(std::size_t q=0;q<active.size();q+=2){if(q+1<active.size()){auto m=active[q]+active[q+1];current+=m;next.push_back(m);}else next.push_back(active[q]);} active.swap(next);}
            }
            ++e623Runs33to64; if(useSlidingTwoOfThree) ++e623CurrentGateHits; e623ExactWrites += exact; e623CurrentWrites += current;
            if(current){ auto saved=current-exact; if(saved*100 >= current*5) ++e623Savings5; if(saved*100 >= current*10) ++e623Savings10; if(saved*100 >= current*15) ++e623Savings15; if(saved*100 >= current*20) ++e623Savings20; }
        }
    }
    #endif

    const bool useE623CompactExact = !useSlidingTwoOfThree && reconstructedRuns >= 33 && reconstructedRuns <= 64;
    if (useE623CompactExact) {
        detail::mergeRunsE618InternalWriteTree(tmp, arr, std::move(runStart), less);
    } else if (useSlidingTwoOfThree) {
#ifdef JESSESORT_E618_TELEMETRY
        if (reconstructedRuns > 32) {
            ++e618ExtendedSchedulerHits;
            e618ExtendedSchedulerRuns += reconstructedRuns;

            // Diagnostic-only cost comparison after matching E375/E601 ordered-boundary
            // elimination. Both totals count one full interval write per internal merge.
            auto diagStart = runStart;
            if (diagStart.size() > 2) {
                std::size_t write = 1;
                for (std::size_t r = 1; r + 1 < diagStart.size(); ++r) {
            ...
            const std::size_t drc = diagStart.size() - 1;
            if (drc > 1) {
                unsigned long long dc[64][64]{};
                unsigned long long pref[65]{};
                for (std::size_t r = 0; r < drc; ++r)
                    pref[r + 1] = pref[r] + (diagStart[r + 1] - diagStart[r]);
                for (std::size_t len = 2; len <= drc; ++len) {
                    for (std::size_t i = 0; i + len <= drc; ++i) {
                        const std::size_t j = i + len - 1;
                        unsigned long long best = ~0ULL / 4ULL;
                        const auto interval = pref[j + 1] - pref[i];
                        for (std::size_t k = i; k < j; ++k)
                            best = std::min(best, interval + dc[i][k] + dc[k + 1][j]);
                        dc[i][j] = best;
                    }
                }
                e618ExactInternalWrites += dc[0][drc - 1];

                std::vector<unsigned long long> active;
                active.reserve(drc);
                for (std::size_t r = 0; r < drc; ++r)
                    active.push_back(diagStart[r + 1] - diagStart[r]);
                unsigned long long e375 = 0;
                while (active.size() > 1) {
                    if (active.size() == 2) {
                        const auto m = active[0] + active[1];
                        e375 += m; active.assign(1, m); break;
                    }
                    std::size_t i = 0; bool any = false;
                    while (i + 2 < active.size()) {
                        const auto ab = active[i] + active[i + 1];
                        const auto bc = active[i + 1] + active[i + 2];
                        const std::size_t pos = (ab <= bc) ? i : i + 1;
                        const auto m = active[pos] + active[pos + 1];
                        e375 += m;
                        active[pos] = m;
                        active.erase(active.begin() + static_cast<std::ptrdiff_t>(pos + 1));
                        any = true;
                        i = std::min(pos + std::size_t{3}, active.size());
                    }
                    if (!any) break;
                }
                e618E375InternalWrites += e375;
            }
        }
#endif
        // E618: extend E601's exact internal-write objective across the full
        // existing E375-selected domain.  No new routing signal is introduced;
        // only the scheduler used after the established gate changes.
#ifdef JESSESORT_E618_RUNTIME_GATE
        if (reconstructedRuns <= 32 || e618EnableExtendedScheduler) {
            detail::mergeRunsE618InternalWriteTree(
...
// E618: exact alphabetic DP for internal merge writes only. For an interval
// [i,j], each root split pays the interval length once plus child costs.
// Positive interval weights satisfy the quadrangle/monotone-optimum condition,
// so Knuth bounds reduce split search from cubic to quadratic while preserving
// the exact E601 objective. Buffer parity remains outside the DP state.
template <typename T, typename Less>
#if defined(__GNUC__) || defined(__clang__)
__attribute__((noinline, cold))
#endif
    ...
    std::vector<std::size_t> runStart, Less less)
{
    ...
    constexpr std::size_t MAX_RC = 64;
    constexpr unsigned long long INF = ~0ULL / 4ULL;
    unsigned long long cost[MAX_RC][MAX_RC];
    unsigned char split[MAX_RC][MAX_RC];
    unsigned long long prefix[MAX_RC + 1];
    prefix[0] = 0;
    ...
    for (std::size_t len = 2; len <= rc; ++len) {
        for (std::size_t i = 0; i + len <= rc; ++i) {
            ...
            // Knuth monotonicity for this alphabetic interval-weight DP:
            // opt[i][j-1] <= opt[i][j] <= opt[i+1][j].
            const std::size_t lo = static_cast<std::size_t>(split[i][j - 1]);
            const std::size_t hi = std::min<std::size_t>(
                j - 1, static_cast<std::size_t>(split[i + 1][j]));
            assert(lo >= i && lo <= hi && hi < j);
