#ifndef JESSESORT_E589_BACKEND_RESCUE_AB_H
#define JESSESORT_E589_BACKEND_RESCUE_AB_H
...
namespace jessesort::experimental::e589_backend_rescue_ab {
...
std::vector<std::size_t> reconstructFrozenBlueprintWithSingleOverflowRun(
    ...
    bool directHighEntropyOverflow = false
) {
    ...
    if (overflowCount != 0) {
        if (directHighEntropyOverflow) {
            simulated_legacy::highEntropyQuickSortProduction(
                tmp.data() + normalCount, overflowCount,
                2 * static_cast<int>(std::bit_width(overflowCount)), less,
                false, T{});
        } else if (patienceSortOverflow) {
...
template <typename T, typename Less = std::less<T>>
void sortImpl(std::vector<T>& arr, Less less, bool bidirectionalBranchlessMerge, bool naturalRunRoute = false, bool enableSpecializedRoutes = false, bool patienceSortOverflow = false, bool enableCoherentValuePileCache = true, bool directHighEntropyOverflow = false, bool enableOppositeOverflowRescue = false) {
    ...
    const bool randomPrefix =
        simulated_early_freeze_legacy::insertionPrefixLooksRandomLike(
            arr, less, 64, 6);
    auto sim = enableOppositeOverflowRescue && randomPrefix
        ? simulated_early_freeze_legacy::simulatePatienceInsertionBlueprintEarlyFreezeOppositeOverflowRescue(
        ...
        : simulated_early_freeze_legacy::simulatePatienceInsertionBlueprintEarlyFreeze(
            arr, less, simulated_early_freeze_legacy::FreezePolicy::power2_single_overflow,
            naturalRunRoute, false, enableCoherentValuePileCache, 50, false, true);
    ...
    const bool routedDirectHighEntropyOverflow = directHighEntropyOverflow &&
        randomPrefix && sim.overflowCount != 0 &&
        sim.overflowCount * 10 >= arr.size();
    std::vector<T> tmp;
    auto runStart = reconstructFrozenBlueprintWithSingleOverflowRun(
        ...
        sim.overflowCount, tmp, less, routedPatienceOverflow, routedDirectHighEntropyOverflow);
