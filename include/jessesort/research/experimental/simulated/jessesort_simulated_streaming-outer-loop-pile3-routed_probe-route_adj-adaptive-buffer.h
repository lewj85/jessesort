#ifndef JESSESORT_E489_SIMULATED_STREAMING_OUTER_LOOP_PILE3_H
#define JESSESORT_E489_SIMULATED_STREAMING_OUTER_LOOP_PILE3_H
...
namespace jessesort::experimental::simulated_streaming_outer_loop_pile3 {
...
void sort(std::vector<T>& arr, Less less = Less{}) {
    ...
    while (pos < n) {
        ...
        // E489 shared streaming router: one pile is committed above; exactly
        // two piles remain on the bounded structured path; 3+ piles hand the
        // entire remaining suffix to the maintained simulated pillar. This is
        // the same categorical 1/2/3+ rule tested physically in E488.
        if (pr.piles >= 3) {
