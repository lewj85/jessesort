#ifndef JESSESORT_EXPERIMENTAL_SIMULATED_FROZEN_OPPOSITE_OVERFLOW_RESCUE_H
#define JESSESORT_EXPERIMENTAL_SIMULATED_FROZEN_OPPOSITE_OVERFLOW_RESCUE_H

#include <jessesort/detail/common/tiny_sort.h>
#include <jessesort/detail/decomposition/frozen/early_freeze_core.h>
#include <jessesort/detail/pipelines/reference/frozen_single.h>
...
namespace jessesort::experimental::simulated_frozen_opposite_overflow_rescue {
...
// Experimental frozen-single entry point: rescue only a would-be overflow into an existing opposite-game pile.
...
void sortImpl(std::vector<T>& arr, Less less, bool bidirectionalBranchlessMerge, bool naturalRunRoute = false, bool enableSpecializedRoutes = false, bool patienceSortOverflow = false, bool enableCoherentValuePileCache = true) {
    ...
    auto sim = simulated_early_freeze_legacy::simulatePatienceInsertionBlueprintEarlyFreezeOppositeOverflowRescue(
...
void sort(std::vector<T>& arr, Less less = Less{}) {
    ...
    // Preserve the production specialized preroutes before paying for the
    // E587 random-prefix admission probe. Once this check has failed, both
    // downstream sortImpl calls disable it to avoid repeating the work.
    if constexpr (simulated_legacy::specializedIntegralEligible<T, Less>) {
        bool prefixMayMix = true;
        if (arr.size() >= 4) {
            const bool asc = less(arr[0], arr[1]) && less(arr[1], arr[2]) && less(arr[2], arr[3]);
    ...
    bool rescueRoute = false;
    if (arr.size() >= 50000) {
        std::size_t prefixPiles = 0;
        rescueRoute = simulated_early_freeze_legacy::insertionPrefixLooksRandomLike(
            arr, less, 64, 6, &prefixPiles);
    }

    if (!rescueRoute) {
        jessesort::simulated_early_freeze_single_overflow_legacy::sortImpl(
            arr, less, true, true, false, true, true);
        return;
    }
    sortImpl(arr, less, true, true, false, true, true);
}
