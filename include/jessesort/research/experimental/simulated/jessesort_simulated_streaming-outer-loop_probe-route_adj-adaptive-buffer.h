#ifndef JESSESORT_E485_SIMULATED_STREAMING_OUTER_LOOP_H
#define JESSESORT_E485_SIMULATED_STREAMING_OUTER_LOOP_H
...
namespace jessesort::experimental::simulated_streaming_outer_loop {
...
inline ProbeResult<T, Less> probeDualPatience(std::span<T> a, Less less) {
    ...
    out.piles = ascTails.size() + descTails.size();
...
void sort(std::vector<T>& arr, Less less = Less{}) {
    ...
    while (pos < n) {
        ...
        if (pr.piles == 1) {
            ...
            // A one-pile probe should already be monotone under the chosen game.
            // Keep this safety net in the experimental baseline so a bad proof
            // cannot silently corrupt the run list.
        ...
        // First baseline: unresolved probe geometry delegates a bounded span to
