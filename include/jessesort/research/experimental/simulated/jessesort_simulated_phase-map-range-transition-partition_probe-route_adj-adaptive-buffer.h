#ifndef JESSESORT_SIMULATED_PHASE_MAP_RANGE_TRANSITION_PARTITION_PROBE_ROUTED_ADJACENT_ADAPTIVE_BUFFERED_H
#define JESSESORT_SIMULATED_PHASE_MAP_RANGE_TRANSITION_PARTITION_PROBE_ROUTED_ADJACENT_ADAPTIVE_BUFFERED_H

#include <jessesort/research/experimental/simulated/jessesort_simulated_phase-map-range_probe-route_adj-adaptive-buffer.h>

namespace jessesort::simulated_phase_map_range_transition_partition {
...
bool shouldForceTransitionPartition(const std::vector<T>& a,
    ...
    if constexpr (!jessesort::simulated_phase_range_engine::specializedIntegralEligible<T, Less>) {
        ...
        return last == RouteState::Entropy;
    }
}
