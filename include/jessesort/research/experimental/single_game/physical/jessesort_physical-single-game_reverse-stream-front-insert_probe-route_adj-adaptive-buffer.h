#ifndef JESSESORT_EXPERIMENTAL_PHYSICAL_SINGLE_GAME_REVERSE_STREAM_FRONT_INSERT_PROBE_ROUTED_ADJACENT_ADAPTIVE_BUFFERED_H
#define JESSESORT_EXPERIMENTAL_PHYSICAL_SINGLE_GAME_REVERSE_STREAM_FRONT_INSERT_PROBE_ROUTED_ADJACENT_ADAPTIVE_BUFFERED_H
...
namespace jessesort::physical_single_game_reverse_front {

// Experimental reverse-stream + front-insert hybrid.  The maintained
// single-game physical mechanics are preserved, but every maximal exact
// non-increasing source run encountered after the ordinary prefix/probe front end
// is consumed right-to-left through the same ascending pile game.  The source
// block is never physically reversed and no minimum run length is tuned.
...
void sortImpl(std::vector<T>& arr, Less less, bool bidirectionalBranchlessMerge,
    ...
    // Logical O(1) front extensions. Each segment stores prepends in arrival
    // order, so flatten emits it in reverse before the ordinary pile body.
    std::vector<std::vector<T>> ascFrontSegments;
    ...
    bool previousWasFrontInsert = false;
    ...
    auto routeGame = [&](T& value) -> bool {
        if (postPrefixDirectionOverride) {
            ...
            previousWasFrontInsert = false;
            return false;
        }
        if (less(*previousValue, value)) { descendingMode = false; previousWasFrontInsert = false; return false; }
        if (less(value, *previousValue)) { descendingMode = false; previousWasFrontInsert = false; return false; }
        // The adj-equivalent shortcut appends to the previous pile's back.
        // A front insertion makes previousValue a pile head, so one equivalent
        // successor must take the ordinary placement path to preserve pile order.
        if (previousWasFrontInsert) { previousWasFrontInsert = false; return false; }
        return true;
    };
    ...
    // Threshold-free exact directional visitor. Discovery examines untouched
    // adjacent source values before any value in the candidate run is moved.
    // A descending adjacency opens a maximal non-increasing run, which is then
    // visited from end to begin so ordinary ascending-pile insertion sees it in
    // normalized order. All other values retain source-order visitation.
    auto tryReverseContextFrontInsert = [&](std::size_t sourceIndex) -> bool {
        T& value = arr[sourceIndex];
        if (ascPiles.empty()) return false;
        // Tail insertion keeps priority. Front extension is considered only when
        // ordinary ascending Patience would otherwise create a new pile.
        if (findAscendingPile(ascBaseArray, value, less) != ascPiles.size()) return false;

        if (ascFrontSegments.size() < ascPiles.size())
            ascFrontSegments.resize(ascPiles.size());
        auto logicalHead = [&](std::size_t p) -> const T& {
            return ascFrontSegments[p].empty()
                ? ascPiles[p].front() : ascFrontSegments[p].back();
        };
        std::size_t best = ascPiles.size();
        for (std::size_t p = 0; p < ascPiles.size(); ++p) {
            const T& head = logicalHead(p);
            // Legal ascending front extension: value <= current logical head.
            if (less(head, value)) continue;
            if (best == ascPiles.size() || less(head, logicalHead(best))) best = p;
        }
        if (best == ascPiles.size()) return false;

        ascFrontSegments[best].push_back(std::move(value));
        descendingMode = false;
        postPrefixDirectionOverride = false;
        lastAscPile = best;
        previousValue = &ascFrontSegments[best].back();
        previousWasFrontInsert = true;
        return true;
    };

    auto visitExactDirectional = [&](auto&& consume) {
        std::size_t i = processStart;
        while (i < n) {
            if (i + 1 < n && less(arr[i + 1], arr[i])) {
                std::size_t end = i + 2;
                while (end < n && !less(arr[end - 1], arr[end])) ++end;
                for (std::size_t j = end; j-- > i;) {
                    if (!tryReverseContextFrontInsert(j)) consume(j);
                }
                i = end;
            } else {
                consume(i);
    ...
    if (e183BoundaryCandidate) {
        visitExactDirectional([&](std::size_t i) {
            T& value = arr[i];
            bool equivalent = false;
            if (previousValue != nullptr) {
                if (less(*previousValue, value)) { descendingMode = false; previousWasFrontInsert = false; }
                else if (less(value, *previousValue)) { descendingMode = false; previousWasFrontInsert = false; }
                else if (previousWasFrontInsert) { previousWasFrontInsert = false; }
    ...
    if (ascFrontSegments.size() < ascPiles.size())
        ascFrontSegments.resize(ascPiles.size());
    for (std::size_t p = 0; p < ascPiles.size(); ++p) {
        auto& front = ascFrontSegments[p];
        for (auto it = front.rbegin(); it != front.rend(); ++it)
            flat.push_back(std::move(*it));
        for (auto& value : ascPiles[p]) flat.push_back(std::move(value));
        ends.push_back(flat.size());
    }
