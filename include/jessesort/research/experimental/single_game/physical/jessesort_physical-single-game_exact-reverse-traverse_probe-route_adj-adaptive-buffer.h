#ifndef JESSESORT_EXPERIMENTAL_PHYSICAL_SINGLE_GAME_EXACT_REVERSE_TRAVERSE_PROBE_ROUTED_ADJACENT_ADAPTIVE_BUFFERED_H
#define JESSESORT_EXPERIMENTAL_PHYSICAL_SINGLE_GAME_EXACT_REVERSE_TRAVERSE_PROBE_ROUTED_ADJACENT_ADAPTIVE_BUFFERED_H
...
namespace jessesort::physical_single_game_exact_reverse {

// Experimental physical single-game exact directional traversal.  The maintained
    ...
    auto visitExactDirectional = [&](auto&& consume) {
        ...
        while (i < n) {
            if (i + 1 < n && less(arr[i + 1], arr[i])) {
                ...
                for (std::size_t j = end; j-- > i;) consume(j);
                i = end;
            } else {
                consume(i);
                ++i;
