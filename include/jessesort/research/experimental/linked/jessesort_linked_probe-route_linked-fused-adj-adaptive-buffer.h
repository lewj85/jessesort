#ifndef JESSESORT_E229_JESSESORT_LINKED_PROBE_ROUTED_LINKED_FUSED_ADJACENT_ADAPTIVE_BUFFERED_H
#define JESSESORT_E229_JESSESORT_LINKED_PROBE_ROUTED_LINKED_FUSED_ADJACENT_ADAPTIVE_BUFFERED_H
...
namespace jessesort::linked_legacy {
...
SimulatedInsertionResult<T> simulatePatienceInsertionBlueprintImpl(
    ...
    const bool confirmedShortSameDirectionPrefix =
        ...
        jessesort::linked_legacy::confirmedShortSameDirectionPrefix(
...
void sortImplCore(std::vector<T>& arr, Less less, bool enableNaturalRunRoute,
    ...
    static_assert(std::is_copy_constructible_v<T> && std::is_copy_assignable_v<T>,
                  "jessesort::linked_legacy::sort requires copyable values because pile tails are stored by value");
    static_assert(std::is_move_constructible_v<T> && std::is_move_assignable_v<T>,
                  "jessesort::linked_legacy::sort requires movable values for merging");
    ...
    jessesort::simulated_legacy::SimulatedInsertionResult<T> sim =
        jessesort::simulated_legacy::simulatePatienceInsertionBlueprint(
    ...
    const bool useRandomBranchlessMerge =
        jessesort::simulated_legacy::shouldUseRandomBranchlessMerge(sim, arr.size());
    ...
    if (linkedRoutingMode == 8) {
        ...
        const std::size_t localityRootN = static_cast<std::size_t>(
            std::sqrt(static_cast<double>(arr.size())));
        const bool compactStructuredLocality =
            probeSpans <= 4 && runCount * 4 <= localityRootN;
        if (compactStructuredLocality) {
        ...
        if (useLinkedReconstruction) {
            ...
            runStart = jessesort::simulated_legacy::reconstructTaggedBlueprintNormalizedForSimulated(
