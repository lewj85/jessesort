#ifndef JESSESORT_E229_JESSESORT_LINKED_DIRECT_MERGE_PROBE_ROUTED_LINKED_FUSED_ADJACENT_ADAPTIVE_BUFFERED_H
#define JESSESORT_E229_JESSESORT_LINKED_DIRECT_MERGE_PROBE_ROUTED_LINKED_FUSED_ADJACENT_ADAPTIVE_BUFFERED_H
...
namespace jessesort::linked_direct_merge {
...
struct SimulatedInsertionResult {
    ...
    // linked/E191 native linked blueprint. During Patience insertion blueprint[i]
    // is immediately converted from a transient pile tag into the index of the
    // next source element in that run's reconstruction order. This keeps the
    // same 4-byte/element metadata footprint as simulated instead of storing tags+links.
    bool nativeLinksEnabled = false;
    std::vector<uint32_t> nativeAscHeads;
    std::vector<uint32_t> nativeAscEnds;
    std::vector<uint32_t> nativeDescHeads;
};
...
inline constexpr uint32_t NATIVE_LINK_NIL = 0xffffffffu;

template <typename T>
inline void recordNativeLinkAssignment(
    SimulatedInsertionResult<T>& result, std::size_t originalIndex
) {
    if (!result.nativeLinksEnabled) return;
    const uint32_t tag = result.blueprint[originalIndex];
    const std::size_t p = static_cast<std::size_t>(localPileId(tag));
    if (isDescTag(tag)) {
        if (result.nativeDescHeads.size() <= p)
            result.nativeDescHeads.resize(p + 1, NATIVE_LINK_NIL);
        result.blueprint[originalIndex] = result.nativeDescHeads[p];
        result.nativeDescHeads[p] = static_cast<uint32_t>(originalIndex);
    } else {
        if (result.nativeAscHeads.size() <= p) {
            result.nativeAscHeads.resize(p + 1, NATIVE_LINK_NIL);
            result.nativeAscEnds.resize(p + 1, NATIVE_LINK_NIL);
        }
        if (result.nativeAscHeads[p] == NATIVE_LINK_NIL) {
            result.nativeAscHeads[p] = static_cast<uint32_t>(originalIndex);
        } else {
            result.blueprint[result.nativeAscEnds[p]] = static_cast<uint32_t>(originalIndex);
        }
        result.blueprint[originalIndex] = NATIVE_LINK_NIL;
        result.nativeAscEnds[p] = static_cast<uint32_t>(originalIndex);
    }
}

template <typename T>
inline void recordNativeLinkRange(
    SimulatedInsertionResult<T>& result, std::size_t begin, std::size_t end
) {
    if (!result.nativeLinksEnabled) return;
    for (std::size_t i = begin; i < end; ++i) recordNativeLinkAssignment(result, i);
}
...
JESSESORT_CONTINUE_NOINLINE void continuePatienceInsertionValleyRescue(
    ...
    auto process = [&](const T& previous, const T& value, std::size_t i) {
        ...
        if (asc) descendingMode = false;
            ...
            recordNativeLinkAssignment(result, i);
            return;
        }
        if (descendingMode) {
...
inline void continuePatienceInsertionNaturalRuns(
    ...
    auto insertOne = [&](std::size_t j) {
        ...
        recordNativeLinkAssignment(result, j);
    };
        ...
        if (len >= minRunLength) {
            if (asc) {
                ...
                if (firstPile == lastPile) {
                    ...
                    recordNativeLinkRange(result, i, end);
...
SimulatedInsertionResult<T> simulatePatienceInsertionBlueprintImpl(
    ...
    result.nativeLinksEnabled = enableNativeLinks;
    ...
    const bool confirmedShortSameDirectionPrefix =
        ...
        jessesort::linked_direct_merge::confirmedShortSameDirectionPrefix(
    ...
    if (materializePrefix) {
        ...
        if (prefixDirection == PrefixDirection::Ascending) {
            ...
            recordNativeLinkRange(result, 0, prefixEnd);
            descendingMode = false;
        } else {
        ...
        if (prefixDirection == PrefixDirection::Descending) {
            ...
            recordNativeLinkAssignment(result, 0);
            descendingMode = true;
        } else {
...
SimulatedInsertionResult<T> simulatePatienceInsertionBlueprint(
    ...
    bool enableNaturalRunRoute = false,
    bool enableCoherentValuePileCache = false,
    bool enableNativeLinks = true
) {
    return simulatePatienceInsertionBlueprintImpl<true>(
        arr, less, enableEarlyRandomInsertionRoute, enableNaturalRunRoute,
        enableCoherentValuePileCache, enableNativeLinks);
}

template <typename T>
std::vector<std::size_t> reconstructNativeLinkedBlueprintToTemp(
    const std::vector<T>& arr,
    const SimulatedInsertionResult<T>& sim,
    std::vector<T>& tmp
) {
    const std::size_t n = arr.size();
    const std::size_t numDescPiles = sim.descCounts.size();
    const std::size_t numAscPiles = sim.ascCounts.size();
    const std::size_t numRuns = numDescPiles + numAscPiles;
    std::vector<std::size_t> start(numRuns + 1, 0);
    std::size_t out = 0;
    for (std::size_t p = 0; p < numDescPiles; ++p) { start[p] = out; out += sim.descCounts[p]; }
    for (std::size_t p = 0; p < numAscPiles; ++p) { const std::size_t r = numDescPiles + p; start[r] = out; out += sim.ascCounts[p]; }
    start[numRuns] = out;
    assert(out == n);
    if constexpr (std::is_default_constructible_v<T>) tmp.resize(n);
    else tmp = arr;
    for (std::size_t p = 0; p < numDescPiles; ++p) {
        std::size_t pos = start[p];
        uint32_t i = sim.nativeDescHeads[p];
        while (i != NATIVE_LINK_NIL) { tmp[pos++] = arr[i]; i = sim.blueprint[i]; }
        assert(pos == start[p + 1]);
    }
    for (std::size_t p = 0; p < numAscPiles; ++p) {
        const std::size_t r = numDescPiles + p;
        std::size_t pos = start[r];
        uint32_t i = sim.nativeAscHeads[p];
        while (i != NATIVE_LINK_NIL) { tmp[pos++] = arr[i]; i = sim.blueprint[i]; }
        assert(pos == start[r + 1]);
    }
    return start;
}


// E192 routed post-built linked reconstruction. Build links from the ordinary
// packed tags only after insertion has completed. Descending piles prepend;
// ascending piles append so reconstruction follows every run forward.
template <typename T>
std::vector<std::size_t> reconstructPostBuiltLinkedBlueprintToTemp(
    const std::vector<T>& arr,
    std::vector<uint32_t> blueprint,
    ...
    const std::size_t n = arr.size();
    const std::size_t nd = descCounts.size(), na = ascCounts.size(), nr = nd + na;
    std::vector<std::size_t> start(nr + 1, 0);
    std::size_t out = 0;
    for (std::size_t p = 0; p < nd; ++p) { start[p] = out; out += descCounts[p]; }
    for (std::size_t p = 0; p < na; ++p) { start[nd + p] = out; out += ascCounts[p]; }
    start[nr] = out;
    assert(out == n);

    // E198 locality candidate: reuse the existing 4-byte tag blueprint as the
    // next-link array. During the forward conversion pass, blueprint[i] is read
    // as its original tag before being overwritten; append updates only touch
    // earlier elements, so unread tags remain intact. This removes the second
    // n-element metadata allocation used by the E193 forced-linked baseline.
    std::vector<uint32_t> heads(nr, NATIVE_LINK_NIL);
    std::vector<uint32_t> tails(nr, NATIVE_LINK_NIL);
    for (uint32_t i = 0; i < static_cast<uint32_t>(n); ++i) {
        ...
        const std::size_t r = desc ? local : nd + local;
        if (desc) {
            blueprint[i] = heads[r];
            heads[r] = i;
            if (tails[r] == NATIVE_LINK_NIL) tails[r] = i;
        } else {
            if (heads[r] == NATIVE_LINK_NIL) heads[r] = i;
            else blueprint[tails[r]] = i;
            blueprint[i] = NATIVE_LINK_NIL;
            tails[r] = i;
        }
    }
    ...
    for (std::size_t r = 0; r < nr; ++r) {
        std::size_t pos = start[r];
        for (uint32_t i = heads[r]; i != NATIVE_LINK_NIL; i = blueprint[i]) tmp[pos++] = arr[i];
        assert(pos == start[r + 1]);
    }
    return start;
}
...
// E199 candidate: combine E198 in-place tag->link conversion with linked
// reconstruction / first-merge fusion. The blueprint vector becomes the next
// array in place, then adjacent linked runs are merged directly to tmp.
template <typename T, typename Less = std::less<T>>
std::vector<std::size_t> reconstructInPlaceLinkedAndFuseFirstMerge(
    const std::vector<T>& arr,
    std::vector<uint32_t> blueprint,
    const std::vector<std::size_t>& ascCounts,
    const std::vector<std::size_t>& descCounts,
    ...
    ends.reserve((nr + 1) / 2);
    auto emit = [&](uint32_t i, std::size_t& pos) {
        while (i != NATIVE_LINK_NIL) { tmp[pos++] = arr[i]; i = blueprint[i]; }
    };
    std::size_t r = 0;
    for (; r + 1 < nr; r += 2) {
        uint32_t i = heads[r], j = heads[r + 1];
        std::size_t pos = start[r];
        if (!less(arr[heads[r + 1]], arr[tails[r]])) {
            emit(i, pos); emit(j, pos);
        } else if (!less(arr[heads[r]], arr[tails[r + 1]])) {
            emit(j, pos); emit(i, pos);
        } else {
            while (i != NATIVE_LINK_NIL && j != NATIVE_LINK_NIL) {
                if (less(arr[j], arr[i])) { tmp[pos++] = arr[j]; j = blueprint[j]; }
                else { tmp[pos++] = arr[i]; i = blueprint[i]; }
            }
            emit(i, pos); emit(j, pos);
        }
        assert(pos == start[r + 2]);
        ends.push_back(start[r + 2]);
    }
    if (r < nr) {
        std::size_t pos = start[r];
        emit(heads[r], pos);
        assert(pos == start[r + 1]);
        ends.push_back(start[r + 1]);
    }
    return ends;
}


// E200 locality candidate: compress consecutive source elements with the same
// pile tag into linked span nodes. Links therefore connect contiguous spans
// rather than individual elements. Structured low-run inputs avoid an O(n)
// next-link materialization pass, while reconstruction still follows a linked
// representation. Adjacent span-linked runs are fused through merge level 1.
template <typename T, typename Less = std::less<T>>
std::vector<std::size_t> reconstructSpanLinkedAndFuseFirstMerge(
    const std::vector<T>& arr,
    ...
    struct SpanNode { uint32_t begin, end, next; bool reverse; };
    const std::size_t n = arr.size();
    const std::size_t nd = descCounts.size(), na = ascCounts.size(), nr = nd + na;
    std::vector<std::size_t> start(nr + 1, 0);
    std::size_t out = 0;
    ...
    nodes.reserve(std::min<std::size_t>(n, nr * 4 + 16));
    std::vector<uint32_t> heads(nr, NATIVE_LINK_NIL), tails(nr, NATIVE_LINK_NIL);
    for (uint32_t i = 0; i < static_cast<uint32_t>(n); ) {
        const uint32_t tag = tags[i];
        uint32_t j = i + 1;
        while (j < static_cast<uint32_t>(n) && tags[j] == tag) ++j;
        const bool desc = isDescTag(tag);
        const std::size_t local = static_cast<std::size_t>(localPileId(tag));
        const std::size_t r = desc ? local : nd + local;
        const uint32_t ni = static_cast<uint32_t>(nodes.size());
        nodes.push_back(SpanNode{i, j, NATIVE_LINK_NIL, desc});
        if (desc) {
            nodes[ni].next = heads[r];
            heads[r] = ni;
            if (tails[r] == NATIVE_LINK_NIL) tails[r] = ni;
        } else {
            if (heads[r] == NATIVE_LINK_NIL) heads[r] = ni;
            else nodes[tails[r]].next = ni;
            tails[r] = ni;
        }
        i = j;
    }
    ...
    struct Cursor {
        const std::vector<SpanNode>* nodes = nullptr;
        uint32_t node = NATIVE_LINK_NIL;
        uint32_t pos = 0;
        bool valid() const { return node != NATIVE_LINK_NIL; }
        uint32_t index() const { return pos; }
        void reset(uint32_t h) {
            node = h;
            if (node != NATIVE_LINK_NIL) {
                const auto& s = (*nodes)[node];
                pos = s.reverse ? s.end - 1u : s.begin;
            }
        }
        void advance() {
            const auto& s = (*nodes)[node];
            if (s.reverse) {
                if (pos > s.begin) { --pos; return; }
            } else {
                if (pos + 1u < s.end) { ++pos; return; }
            }
            node = s.next;
            if (node != NATIVE_LINK_NIL) {
                const auto& t = (*nodes)[node];
                pos = t.reverse ? t.end - 1u : t.begin;
            }
        }
    };
    auto firstIndex = [&](std::size_t r) -> uint32_t {
        const auto& s0 = nodes[heads[r]];
        return s0.reverse ? s0.end - 1u : s0.begin;
    };
    auto lastIndex = [&](std::size_t r) -> uint32_t {
        const auto& st = nodes[tails[r]];
        return st.reverse ? st.begin : st.end - 1u;
    };
    auto emit = [&](Cursor& c, std::size_t& pos) {
        while (c.valid()) { tmp[pos++] = arr[c.index()]; c.advance(); }
    };
    ...
    std::size_t r = 0;
    for (; r + 1 < nr; r += 2) {
        Cursor a{&nodes}, b{&nodes}; a.reset(heads[r]); b.reset(heads[r + 1]);
        std::size_t pos = start[r];
        if (!less(arr[firstIndex(r + 1)], arr[lastIndex(r)])) {
            emit(a, pos); emit(b, pos);
        } else if (!less(arr[firstIndex(r)], arr[lastIndex(r + 1)])) {
            emit(b, pos); emit(a, pos);
        } else {
            while (a.valid() && b.valid()) {
                if (less(arr[b.index()], arr[a.index()])) { tmp[pos++] = arr[b.index()]; b.advance(); }
                else { tmp[pos++] = arr[a.index()]; a.advance(); }
            }
            emit(a, pos); emit(b, pos);
        }
        assert(pos == start[r + 2]);
    ...
    if (r < nr) {
        Cursor a{&nodes}; a.reset(heads[r]);
        std::size_t pos = start[r]; emit(a, pos);
        assert(pos == start[r + 1]);
        ends.push_back(start[r + 1]);
    }
    return ends;
...
__attribute__((noinline))
#endif
inline bool shouldUseElementLinksFromFinalGeometry(
    const std::vector<std::size_t>& ascCounts,
    const std::vector<std::size_t>& descCounts,
    std::size_t n,
    std::size_t runCount
) {
    const std::size_t ascRuns = ascCounts.size();
    const std::size_t descRuns = descCounts.size();
    const std::size_t smallerGame = std::min(ascRuns, descRuns);
    const std::size_t largerGame = std::max(ascRuns, descRuns);
    if (largerGame == 0 || smallerGame * 5 < largerGame)
        return false;

    __extension__ typedef unsigned __int128 Wide;
    Wide sumSq = 0;
    for (std::size_t count : ascCounts)
        sumSq += static_cast<Wide>(count) * count;
    for (std::size_t count : descCounts)
        sumSq += static_cast<Wide>(count) * count;
    const Wide dispersedLimit = static_cast<Wide>(12) * n * runCount;
    return sumSq <= dispersedLimit;
}
...
void sortImplCore(std::vector<T>& arr, Less less, bool enableNaturalRunRoute,
                  ...
                  bool enableCoherentValuePileCache = false,
                  int linkedRoutingMode = 0,
                  bool* linkedUsedOut = nullptr) {
    if (linkedUsedOut) *linkedUsedOut = false;
    static_assert(std::is_copy_constructible_v<T> && std::is_copy_assignable_v<T>,
                  "jessesort::linked_direct_merge::sort requires copyable values because pile tails are stored by value");
    static_assert(std::is_move_constructible_v<T> && std::is_move_assignable_v<T>,
                  "jessesort::linked_direct_merge::sort requires movable values for merging");
    ...
    jessesort::simulated_direct_merge::SimulatedInsertionResult<T> sim =
        jessesort::simulated_direct_merge::simulatePatienceInsertionBlueprint(
            arr, less, true, enableNaturalRunRoute, enableCoherentValuePileCache);
    ...
    const std::size_t runCount = sim.ascCounts.size() + sim.descCounts.size();
    bool useLinkedReconstruction = false;
    if (linkedRoutingMode == 3) {
        // E193 moderate route: preserve E192's final-shape requirements but
        // remove the 20k ceiling so the linked family can be exercised at 100k.
        useLinkedReconstruction = !sim.earlyRandomLike && runCount >= 256;
    } else if (linkedRoutingMode == 2) {
        // E193 always-linked control: whenever Patience reconstruction is reached,
        // use the linked representation regardless of shape. Specialized direct
        // backends and monotone early exits still remain production-faithful.
        useLinkedReconstruction = true;
    } else if (linkedRoutingMode == 1) {
        // E193 loose route: admit every non-random-like Patience shape at all
        // benchmark sizes; remove the E192 size and run-count restrictions.
        useLinkedReconstruction = !sim.earlyRandomLike;
    } else if (linkedRoutingMode == 4) {
        // E194 candidate A: links remain the default. Fall back only for the
        // measured catastrophic long-run family: very few final piles plus a
        // nearly single-direction first-64 prefix. Sawtooth also has a monotone
        // prefix, but its final run count is materially larger, while Alternating
        // has only two runs but a maximally alternating prefix.
        std::size_t directionChanges = 0;
        int previousDirection = 0;
        const std::size_t probeEnd = std::min<std::size_t>(64, arr.size());
        for (std::size_t i = 1; i < probeEnd; ++i) {
            int direction = 0;
            if (less(arr[i - 1], arr[i])) direction = 1;
            else if (less(arr[i], arr[i - 1])) direction = -1;
            if (direction != 0) {
        ...
        const bool catastrophicLongRunShape =
            runCount <= 20 && directionChanges <= 2;
        useLinkedReconstruction = !catastrophicLongRunShape;
    } else if (linkedRoutingMode == 5) {
        // E195 candidate B: start from E194's catastrophic-shape fallback, then
        // distinguish sparse 5%-noise from denser 10%-noise only when the
        // first-64 direction pattern is noise-like. The hypothesis is that
        // noise density is reflected in final run density; monotone/sawtooth
        // and highly alternating families are intentionally outside this gate.
        std::size_t directionChanges = 0;
        int previousDirection = 0;
        const std::size_t probeEnd = std::min<std::size_t>(64, arr.size());
        for (std::size_t i = 1; i < probeEnd; ++i) {
        ...
        const bool catastrophicLongRunShape =
            runCount <= 20 && directionChanges <= 2;
        const bool noiseLikePrefix = directionChanges >= 2 && directionChanges <= 30;
        const std::size_t rootN = static_cast<std::size_t>(
            std::sqrt(static_cast<double>(arr.size())));
        const bool sparseNoiseShape = noiseLikePrefix &&
            runCount * 10 < rootN * 9;
        useLinkedReconstruction = !(catastrophicLongRunShape || sparseNoiseShape);
    } else {
        useLinkedReconstruction =
            arr.size() <= 20000 && !sim.earlyRandomLike && runCount >= 256;
    }
    if (linkedUsedOut) *linkedUsedOut = useLinkedReconstruction;

    std::vector<std::size_t> ends;
    if (linkedRoutingMode == 8) {
        // E201: route only between linked representations. A very low number of
        // tag spans in the first 64 blueprint entries predicts long contiguous
        // source spans, where span-linked reconstruction is substantially better.
        // Fragmented shapes stay on E199's in-place element links.
        std::size_t probeSpans = sim.blueprint.empty() ? 0 : 1;
        const std::size_t probeEnd = std::min<std::size_t>(64, sim.blueprint.size());
        for (std::size_t i = 1; i < probeEnd; ++i)
            probeSpans += sim.blueprint[i] != sim.blueprint[i - 1] ? 1u : 0u;
        if (linkedUsedOut) *linkedUsedOut = true;
        bool useSpan = probeSpans <= 4;
        if (useSpan && shouldUseElementLinksFromFinalGeometry(
                sim.ascCounts, sim.descCounts, arr.size(), runCount))
            useSpan = false;
        if (useSpan) {
            ends = reconstructSpanLinkedAndFuseFirstMerge(
                arr, sim.blueprint, sim.ascCounts, sim.descCounts, tmp, less);
        } else {
            ends = reconstructInPlaceLinkedAndFuseFirstMerge(
                arr, std::move(sim.blueprint), sim.ascCounts, sim.descCounts, tmp, less);
        }
    } else if (linkedRoutingMode == 7) {
        if (linkedUsedOut) *linkedUsedOut = true;
        ends = reconstructSpanLinkedAndFuseFirstMerge(
            arr, sim.blueprint, sim.ascCounts, sim.descCounts, tmp, less);
    } else if (linkedRoutingMode == 6) {
        if (linkedUsedOut) *linkedUsedOut = true;
        ends = reconstructInPlaceLinkedAndFuseFirstMerge(
            arr, std::move(sim.blueprint), sim.ascCounts, sim.descCounts, tmp, less);
    } else {
        std::vector<std::size_t> runStart;
        if (useLinkedReconstruction) {
            runStart = reconstructPostBuiltLinkedBlueprintToTemp(
                arr, std::move(sim.blueprint), sim.ascCounts, sim.descCounts, tmp);
        } else {
            runStart = jessesort::simulated_direct_merge::reconstructTaggedBlueprintNormalizedForSimulated(
                arr, std::move(sim.blueprint), std::move(sim.ascCounts),
                std::move(sim.descCounts), tmp, false, true, true, true);
        }
        ends.assign(runStart.begin() + 1, runStart.end());
    }
...
template <typename T, typename Less = std::less<T>>
void sortModerateLinked(std::vector<T>& arr, Less less = Less{}, bool* linkedUsedOut = nullptr) {
    if (jessesort::detail::tryTinyInsertionSort(arr, less)) { if (linkedUsedOut) *linkedUsedOut = false; return; }
    bool linkedProbeCandidate = false;
    if (arr.size() >= 10000) {
        const std::size_t end = std::min<std::size_t>(64, arr.size());
        std::size_t desc = 0, asc = 0;
        for (std::size_t i = 1; i < end; ++i) {
            if (less(arr[i], arr[i - 1])) ++desc;
            else if (less(arr[i - 1], arr[i])) ++asc;
        }
        linkedProbeCandidate = desc >= 5 && desc <= 15 && asc >= 2 * desc;
    }
    if (!linkedProbeCandidate) {
        if (linkedUsedOut) *linkedUsedOut = false;
        jessesort::simulated_direct_merge::sort(arr, less);
        return;
    }
    sortImplCore(arr, less, true, true, true, true, true, 3, linkedUsedOut);
}

template <typename T, typename Less = std::less<T>>
void sortLooseLinked(std::vector<T>& arr, Less less = Less{}, bool* linkedUsedOut = nullptr) {
    if (jessesort::detail::tryTinyInsertionSort(arr, less)) { if (linkedUsedOut) *linkedUsedOut = false; return; }
    // E193 loose-routing experiment: remove the E192 outer prefix gate and let
    // all production Patience candidates enter linked. Inner routing uses links for
    // every non-random-like reconstructed shape.
    sortImplCore(arr, less, true, true, true, true, true, 1, linkedUsedOut);
}

template <typename T, typename Less = std::less<T>>
void sortAlwaysLinked(std::vector<T>& arr, Less less = Less{}, bool* linkedUsedOut = nullptr) {
    if (jessesort::detail::tryTinyInsertionSort(arr, less)) { if (linkedUsedOut) *linkedUsedOut = false; return; }
    // E193 no-routing baseline: whenever the production flow reaches Patience
    // reconstruction, force linked reconstruction. Direct primitive backends
    // and monotone early exits remain intact because no blueprint exists there.
    sortImplCore(arr, less, true, true, true, true, true, 2, linkedUsedOut);
}

template <typename T, typename Less = std::less<T>>
void sortFusedLinked(std::vector<T>& arr, Less less = Less{}, bool* linkedUsedOut = nullptr) {
    if (jessesort::detail::tryTinyInsertionSort(arr, less)) { if (linkedUsedOut) *linkedUsedOut = false; return; }
    sortImplCore(arr, less, true, true, true, true, true, 6, linkedUsedOut);
}

template <typename T, typename Less = std::less<T>>
void sortSpanLinked(std::vector<T>& arr, Less less = Less{}, bool* linkedUsedOut = nullptr) {
    if (jessesort::detail::tryTinyInsertionSort(arr, less)) { if (linkedUsedOut) *linkedUsedOut = false; return; }
    sortImplCore(arr, less, true, true, true, true, true, 7, linkedUsedOut);
}

template <typename T, typename Less = std::less<T>>
void sortHybridLinked(std::vector<T>& arr, Less less = Less{}, bool* linkedUsedOut = nullptr) {
    if (jessesort::detail::tryTinyInsertionSort(arr, less)) { if (linkedUsedOut) *linkedUsedOut = false; return; }
    sortImplCore(arr, less, true, true, true, true, true, 8, linkedUsedOut);
}

template <typename T, typename Less = std::less<T>>
void sortCatastrophicFallbackLinked(std::vector<T>& arr, Less less = Less{}, bool* linkedUsedOut = nullptr) {
    if (jessesort::detail::tryTinyInsertionSort(arr, less)) { if (linkedUsedOut) *linkedUsedOut = false; return; }
    sortImplCore(arr, less, true, true, true, true, true, 4, linkedUsedOut);
}

template <typename T, typename Less = std::less<T>>
void sortNoiseDensityFallbackLinked(std::vector<T>& arr, Less less = Less{}, bool* linkedUsedOut = nullptr) {
    if (jessesort::detail::tryTinyInsertionSort(arr, less)) { if (linkedUsedOut) *linkedUsedOut = false; return; }
    sortImplCore(arr, less, true, true, true, true, true, 5, linkedUsedOut);
}
...
void sort(std::vector<T>& arr, Less less = Less{}) {
    ...
    if (arr.size() >= 10000) {
        // E635: retain the full E271 dispatcher down to 10k. Relative to E634,
        // E233/long-run ownership is unchanged; this extends only E232 sparse
        // distributed-disorder ownership into the 10k-32k range.
        if (jessesort::simulated_direct_merge::tryStructuralPreRoutesE271(arr, less)) return;
    }
    // E201 retained linked baseline: remain linked whenever Patience reconstruction
    // is reached, but route between two linked layouts based on blueprint locality.
    // Long contiguous tag spans use compact span links; fragmented shapes use
    // in-place element links. No simulated/tag reconstruction fallback is used here.
    sortHybridLinked(arr, less, nullptr);
}
