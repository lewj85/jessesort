#ifndef JESSESORT_DETAIL_RUST_UNSTABLE_NOALLOC_H
#define JESSESORT_DETAIL_RUST_UNSTABLE_NOALLOC_H
...
namespace jessesort::rust_unstable_noalloc_detail {
...
template <std::size_t ScratchBytes, class It>
__attribute__((noinline))
void gatherEvenPositionsLeafScratch(It first, std::size_t n) {
    using T = typename std::iterator_traits<It>::value_type;
    constexpr std::size_t kCapacity = ScratchBytes / sizeof(T);
    using Storage = std::aligned_storage_t<sizeof(T), alignof(T)>;
    std::array<Storage, kCapacity> scratch;
...
template <class It>
__attribute__((noinline))
void gatherEvenPositionsAdaptiveLeaf(It first, std::size_t n) {
    using T = typename std::iterator_traits<It>::value_type;
    // E641: allocate scratch only in a leaf helper, and choose the smallest
    // bounded bucket that can hold this leaf's actual gather span. The
    // recursive deinterleave still stops at the mature 16 KiB capacity, so
    // recursion depth/algorithmic behavior is unchanged from E638.
    if constexpr (2048 / sizeof(T) >= 4) {
        if (n <= 2048 / sizeof(T)) { gatherEvenPositionsLeafScratch<2048>(first, n); return; }
    }
    if constexpr (4096 / sizeof(T) >= 4) {
        if (n <= 4096 / sizeof(T)) { gatherEvenPositionsLeafScratch<4096>(first, n); return; }
    }
    if constexpr (8192 / sizeof(T) >= 4) {
        if (n <= 8192 / sizeof(T)) { gatherEvenPositionsLeafScratch<8192>(first, n); return; }
    }
    if constexpr (12288 / sizeof(T) >= 4) {
        if (n <= 12288 / sizeof(T)) { gatherEvenPositionsLeafScratch<12288>(first, n); return; }
    }
    gatherEvenPositionsLeafScratch<16384>(first, n);
}

template <class It>
void gatherEvenPositionsFirst(It first, std::size_t n) {
    using T = typename std::iterator_traits<It>::value_type;
    constexpr std::size_t kMaxCapacity = 16384 / sizeof(T);
#ifndef JESSESORT_E359_DISABLE_LOCAL_GATHER
    if constexpr (kMaxCapacity >= 4) {
        if (n <= kMaxCapacity) {
            gatherEvenPositionsAdaptiveLeaf(first, n);
            return;
        }
    }
#endif
...
// E731: strict-native retention of E729's size-gated one-pass whole-input
// natural-run prepass. This is admission/order-of-operations only: accepted
// inputs are realized exclusively by the existing strict bounded noalloc
// natural-run machinery below. Rejection is read-only and falls through to
// the unchanged strict pipeline.
template <class T, class Less>
bool tryE729PrepassStrictNoAlloc(std::vector<T>& a, Less less) {
    constexpr std::size_t kPrefix = 64;
    const std::size_t n = a.size();
    ...
    while (i < n) {
        if (runCount == runs.size()) return false;
        ...
        // Generic E729 false-positive bailout: three short early runs whose
        // value ranges overlap are left to the ordinary strict front-end.
        if (runCount >= 2 && runCount <= 3) {
            const auto left = runs[runCount - 2];
            const auto right = runs[runCount - 1];
            const T& l0 = a[left.begin];
            const T& l1 = a[left.end - 1];
            const T& r0 = a[right.begin];
            const T& r1 = a[right.end - 1];
            ...
            if (runCount == 3 && firstTwoOverlap && i <= 8192) return false;
        }
    }
    ...
    bool hasDescendingRun = false;
    for (std::size_t r = 0; r < runCount; ++r)
        hasDescendingRun |= runs[r].descending;
...
void sort(std::vector<T>& a, Less less = Less{}) {
    ...
    // E731 retained: try the strict-native E729 prepass before older front-end
    // probes. Rejection is read-only; success stays within strict bounded noalloc.
    if (tryE729PrepassStrictNoAlloc(a, less)) return;
    ...
    if (a.size() >= 10000) {
        ...
        // E645 strict worst-case contract: the sampled sparse-disorder proof is
        // a performance classifier, not a global bound on unsampled regions.
        // Its bottom-up rotation-merge realization can be O(n log^2 n), so a
        // positive nomination must use the bounded O(n log n) fallback here.
        if (sparseMiddleDensityCandidateNoAlloc(a, less)) {
            jessesort::allocation_free_bounded::fallbackNoAlloc(a, less);
            return;
        }
    }
    ...
    std::size_t runCount = 0;
    if (!jessesort::allocation_free_bounded::discoverNaturalRuns(a, runs, runCount, less)) {
        // E638 Rust-contract fork: fixed descriptor capacity is a hard bound.
        // Do not enter E217 live reclamation because its global worst-case
        // comparison/movement bound has not been established as O(n log n).
        jessesort::allocation_free_bounded::fallbackNoAlloc(a, less);
        return;
    }
    if (jessesort::allocation_free_bounded::cheapMergeGeometryOnly(a, runs, runCount, less)) {
...
namespace jessesort::experimental::strict_e731_legacy {

template <class T, class Less = std::less<T>>
void sort(std::vector<T>& values, Less less = Less{}) {
    jessesort::rust_unstable_noalloc_detail::sort(values, less);
}
