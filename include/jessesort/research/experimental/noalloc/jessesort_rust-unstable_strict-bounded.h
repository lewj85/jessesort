...
namespace jessesort::rust_unstable_candidate {
...
bool hasTwoLaneZigzagStructureNoAlloc(const std::vector<T>& a, Less less, int* evenDirOut = nullptr, int* oddDirOut = nullptr) {
    ...
    auto verifyLaneAfter = [&](int dir, std::size_t strictAt) -> bool {
    ...
    if (!verifyLaneAfter(evenDir, evenStrict) || !verifyLaneAfter(oddDir, oddStrict)) return false;
...
template <class T, class Less>
inline bool sparseDisorderPrefixCandidateNoAlloc(const std::vector<T>& a, Less less) {
    if (a.size() < 10000) return false;
    unsigned prefixInversions = 0;
    for (std::size_t i = 1; i < 8; ++i)
        prefixInversions += static_cast<unsigned>(less(a[i], a[i - 1]));
    return prefixInversions <= 2;
}

template <class T, class Less>
__attribute__((noinline))
bool hasSparseDistributedDisorderWindowsNoAlloc(const std::vector<T>& a, Less less) {
...
bool trySparseDistributedDisorderDirectNoAlloc(std::vector<T>& a, Less less) {
    if (!sparseDisorderPrefixCandidateNoAlloc(a, less)) return false;
    if (!hasSparseDistributedDisorderWindowsNoAlloc(a, less)) return false;
