...
void mergePairLeafBufferedNoAlloc(std::vector<T>& a, std::size_t begin,
    ...
    if constexpr (kCapacity >= 2) {
        ...
        mergeRecursiveLeafBufferedNoAlloc(a.begin() + static_cast<std::ptrdiff_t>(begin),
...
template <class T, class Less>
bool hasTwoLaneZigzagStructureNoAlloc(const std::vector<T>& a, Less less) {
    constexpr std::size_t kMinN = 10000;
...
bool tryTwoLaneZigzagDirectNoAlloc(std::vector<T>& a, Less less) {
    if (!hasTwoLaneZigzagStructureNoAlloc(a, less)) return false;
    const std::size_t n = a.size();
