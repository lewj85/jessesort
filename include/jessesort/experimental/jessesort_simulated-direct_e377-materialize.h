#ifndef JESSESORT_E377_MATERIALIZE_H
#define JESSESORT_E377_MATERIALIZE_H
...
namespace jessesort::simulated_direct_merge_e377 {
...
SimulatedInsertionResult<T> simulatePatienceInsertionBlueprintImpl(
    ...
    const bool confirmedShortSameDirectionPrefix =
        ...
        jessesort::simulated_direct_merge_e377::confirmedShortSameDirectionPrefix(
...
inline unsigned e377TreeGallopTrigger = 7;
inline bool e377UseFixedSlide3 = false;
...
void materializePowerSortTree(
    ...
    mergeTwoAdjacentRunsToDest(
        src, dst, node.begin, node.mid, node.end, less, e377TreeGallopTrigger);
}
...
template <typename T, typename Less>
void e377MaterializeFixed(const std::array<PowerSortMergeNode,127>& nodes, std::size_t idx, bool outArr, std::vector<T>& tmp, std::vector<T>& arr, Less less) {
    const auto& node=nodes[idx];
    if(node.leaf){
        if(outArr) std::move(tmp.begin()+static_cast<std::ptrdiff_t>(node.begin), tmp.begin()+static_cast<std::ptrdiff_t>(node.end), arr.begin()+static_cast<std::ptrdiff_t>(node.begin));
        return;
    }
    e377MaterializeFixed(nodes,node.left,!outArr,tmp,arr,less);
    e377MaterializeFixed(nodes,node.right,!outArr,tmp,arr,less);
    auto& src=outArr?tmp:arr; auto& dst=outArr?arr:tmp;
    mergeTwoAdjacentRunsToDest(src,dst,node.begin,node.mid,node.end,less,7);
}
inline std::size_t e377FixedCopyCost(const std::array<PowerSortMergeNode,127>& nodes,std::size_t idx,bool outArr){
    const auto& n=nodes[idx]; if(n.leaf) return outArr?n.end-n.begin:0;
    return e377FixedCopyCost(nodes,n.left,!outArr)+e377FixedCopyCost(nodes,n.right,!outArr);
}

template <typename T, typename Less>
void mergeRunsE377FixedSlide3(std::vector<T>& tmp,std::vector<T>& arr,std::vector<std::size_t> runStart,Less less){
    const std::size_t n=tmp.size(); if(n==0){arr.clear();return;}
    if(runStart.size()>2){std::size_t w=1;for(std::size_t r=1;r+1<runStart.size();++r){auto b=runStart[r];if(less(tmp[b],tmp[b-1]))runStart[w++]=b;}runStart[w++]=n;runStart.resize(w);}
    std::size_t rc=runStart.size()-1; if(rc==0){arr.clear();return;} if(rc==1){arr.swap(tmp);return;} assert(rc<=64);
    std::array<PowerSortMergeNode,127> nodes{}; std::array<std::size_t,64> active{}; std::size_t nc=0,ac=rc;
    for(std::size_t r=0;r<rc;++r){active[r]=nc;nodes[nc++]=PowerSortMergeNode{runStart[r],runStart[r],runStart[r+1],0,0,true};}
    auto len=[&](std::size_t x){return nodes[x].end-nodes[x].begin;};
    auto mergeAt=[&](std::size_t p){auto l=active[p],rr=active[p+1];assert(nodes[l].end==nodes[rr].begin);auto ni=nc++;nodes[ni]=PowerSortMergeNode{nodes[l].begin,nodes[l].end,nodes[rr].end,l,rr,false};active[p]=ni;for(std::size_t j=p+1;j+1<ac;++j)active[j]=active[j+1];--ac;};
    while(ac>1){if(ac==2){mergeAt(0);break;}std::size_t i=0;bool any=false;while(i+2<ac){auto ab=len(active[i])+len(active[i+1]);auto bc=len(active[i+1])+len(active[i+2]);auto p=(ab<=bc)?i:i+1;mergeAt(p);any=true;i=std::min(p+3,ac);}if(!any)break;}
    auto root=active[0];auto ca=e377FixedCopyCost(nodes,root,true),ct=e377FixedCopyCost(nodes,root,false);bool outArr=ca<=ct;e377MaterializeFixed(nodes,root,outArr,tmp,arr,less);if(!outArr)arr.swap(tmp);
}
...
void sortImplCoreWithCheckpoint(std::vector<T>& arr, Less less, bool enableNaturalRunRoute,
    ...
    if (selectedSchedule == detail::E375Schedule::adjacent) {
        ...
        arr = std::move(tmp);
    } else if (selectedSchedule == detail::E375Schedule::sliding_stride3 && detail::e377UseFixedSlide3) {
        detail::mergeRunsE377FixedSlide3(tmp, arr, std::move(runStart), less);
    } else {
