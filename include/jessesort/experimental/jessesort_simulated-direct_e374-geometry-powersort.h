#ifndef JESSESORT_E374_GEOMETRY_POWERSORT_H
#define JESSESORT_E374_GEOMETRY_POWERSORT_H
...
namespace jessesort::simulated_direct_merge_e374 {
inline bool e374EnablePowerSort = true;
...
SimulatedInsertionResult<T> simulatePatienceInsertionBlueprintImpl(
    ...
    const bool confirmedShortSameDirectionPrefix =
        ...
        jessesort::simulated_direct_merge_e374::confirmedShortSameDirectionPrefix(
...
inline bool shouldUsePowerSortMerge(
    ...
    if (!e374EnablePowerSort || n < 10000 || runStart.size() <= 3) return false;
    const std::size_t runCount = runStart.size() - 1;
    std::size_t maxLength = 0;
    for (std::size_t r = 0; r < runCount; ++r)
        maxLength = std::max(maxLength, runStart[r + 1] - runStart[r]);
    __extension__ typedef unsigned __int128 Wide;
    const Wide concentration = static_cast<Wide>(maxLength) * runCount;
    // E374 diagnostic gate: compact mildly-skewed run sets (natural/block
    // structure) or any strongly concentrated run set (sparse noise / mixed
    // phase). Avoid high-run-count regular/random geometry.
    return runCount >= 4 && runCount <= 32 &&
           concentration * 4 >= static_cast<Wide>(7) * n;
}
...
void sortImplCoreWithCheckpoint(std::vector<T>& arr, Less less, bool enableNaturalRunRoute,
    ...
    if (detail::shouldUsePowerSortMerge(runStart, arr.size())) {
        detail::mergeRunsPowerSortStyle(tmp, arr, std::move(runStart), less);
        return;
    }
...
} // namespace jessesort::simulated_direct_merge_e374_e374
#endif
