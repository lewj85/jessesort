#ifndef JESSESORT_E379_PHYSICAL_GEOMETRY_H
#define JESSESORT_E379_PHYSICAL_GEOMETRY_H

#include <jessesort/tiny_sort.h>
#include <jessesort/jessesort_simulated_probe-routed_adjacent-adaptive-buffered.h>
#include <jessesort/experimental/jessesort_simulated-direct_e375-twoofthree.h>
...
namespace jessesort::actual_piles_e379 {

inline thread_local void (*e379_merge_hook)(const void*, const std::vector<std::size_t>&, std::size_t, std::size_t) = nullptr;
inline thread_local bool e379_enable_stride3 = false;
...
void sortImpl(std::vector<T>& arr, Less less, bool bidirectionalBranchlessMerge,
    ...
    static_assert(std::is_copy_constructible_v<T> && std::is_copy_assignable_v<T>,
                  "jessesort::actual_piles_legacy::sort requires copyable values because pile tails are stored by value");
    static_assert(std::is_move_constructible_v<T> && std::is_move_assignable_v<T>,
                  "jessesort::actual_piles_legacy::sort requires movable values for pile flattening and merging");
    ...
    if (e379_merge_hook) e379_merge_hook(static_cast<const void*>(flat.data()), ends, n, sizeof(T));
    bool useStride3 = false;
    if (e379_enable_stride3 && ends.size() >= 8 && ends.size() <= 32) {
        ...
        if (rc >= 8 && rc <= 32) {
            std::size_t prev=0, mx=0;
            for (std::size_t e : gateEnds) { mx=std::max(mx,e-prev); prev=e; }
            useStride3 = static_cast<unsigned long long>(mx) * rc * 10ULL >
    ...
    if (useStride3) {
        ...
        jessesort::simulated_direct_merge_e375::detail::mergeRunsE375Tree(
            flat, buffer, std::move(starts), less,
            jessesort::simulated_direct_merge_e375::detail::E375Schedule::sliding_stride3);
        arr = std::move(buffer);
    } else {
