#ifndef JESSESORT_E376_SIMULATED_TWO_OF_THREE_H
#define JESSESORT_E376_SIMULATED_TWO_OF_THREE_H
...
namespace jessesort::simulated_legacy_e376 {
...
SimulatedInsertionResult<T> simulatePatienceInsertionBlueprintImpl(
    ...
    const bool confirmedShortSameDirectionPrefix =
        ...
        jessesort::simulated_legacy_e376::confirmedShortSameDirectionPrefix(
...
inline bool e376EnableTwoOfThree = false;
...
void sortImplCore(std::vector<T>& arr, Less less, bool enableNaturalRunRoute,
    ...
    static_assert(std::is_copy_constructible_v<T> && std::is_copy_assignable_v<T>,
                  "jessesort::simulated_legacy_e376::sort requires copyable values because pile tails are stored by value");
    static_assert(std::is_move_constructible_v<T> && std::is_move_assignable_v<T>,
                  "jessesort::simulated_legacy_e376::sort requires movable values for merging");
    ...
    for (std::size_t r = 0; r < reconstructedRuns; ++r)
        maxReconstructedRun = std::max(maxReconstructedRun, runStart[r + 1] - runStart[r]);
    const bool useSlidingTwoOfThree = detail::e376EnableTwoOfThree &&
    ...
    if (useSlidingTwoOfThree) {
        detail::mergeRunsE375Tree(tmp, arr, std::move(runStart), less,
            detail::E375Schedule::sliding_stride3);
    } else {
        std::vector<std::size_t> ends(runStart.begin() + 1, runStart.end());
        mergeRunsAdjacentPairsEnds(tmp, arr, ends, less, useRandomBranchlessMerge, true);
        arr = std::move(tmp);
    }
}
