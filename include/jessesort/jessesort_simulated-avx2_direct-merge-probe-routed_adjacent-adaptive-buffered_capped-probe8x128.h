#ifndef JESSESORT_E229_JESSESORT_SIMULATED_AVX2_DIRECT_MERGE_PROBE_ROUTED_ADJACENT_ADAPTIVE_BUFFERED_CAPPED_PROBE8X128_H
#define JESSESORT_E229_JESSESORT_SIMULATED_AVX2_DIRECT_MERGE_PROBE_ROUTED_ADJACENT_ADAPTIVE_BUFFERED_CAPPED_PROBE8X128_H
...
namespace jessesort::simulated_simd_v7_direct_merge {
...
JESSESORT_CONTINUE_NOINLINE void continuePatienceInsertionCoherentCache(
    ...
    static_assert(jessesort::simulated_direct_merge::specializedIntegralEligible<T, Less>);
...
SimulatedInsertionResult<T> simulatePatienceInsertionBlueprint(
    ...
    // E099: propagate E098's narrow natural-run batching into V7. This is
...
void sortWithProbePileCap(std::vector<T>& arr, std::size_t probePileCap, std::size_t probeValues, Less less, bool bidirectionalBranchlessMerge, bool normalizedReconstruction = true, bool naturalRunRoute = false, bool enableWindowAvx = false, bool enableSpecializedRoutes = false, bool enableCoherentValuePileCache = true, bool enableRawBulkSpanDecode = false) {
    static_assert(std::is_copy_constructible_v<T> && std::is_copy_assignable_v<T>,
                  "jessesort::simulated_simd_v7_direct_merge::sort requires copyable values because pile tails are stored by value");
    static_assert(std::is_move_constructible_v<T> && std::is_move_assignable_v<T>,
                  "jessesort::simulated_simd_v7_direct_merge::sort requires movable values for merging");
    ...
    if constexpr (jessesort::simulated_direct_merge::specializedIntegralEligible<T, Less>) {
        if (enableSpecializedRoutes) {
            ...
            if (prefixMayMix && jessesort::simulated_direct_merge::trySpecializedPrePatienceRoutes(arr, less)) return;
        }
    }
    ...
    if (normalizedReconstruction) {
        runStart = reconstructTaggedBlueprintNormalizedForV7(
            arr,
            std::move(sim.blueprint),
            std::move(sim.ascCounts),
            std::move(sim.descCounts),
